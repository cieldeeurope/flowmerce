            ...
            while (retryCount < 3 && !success) {
                ...
                try {
                    ...
                    const categories = await page.evaluate((): { categoryName: string; url: string }[] => {
                        ...
                        const groups: { selector: string; name: string; pathToken: string }[] = [
                            { selector: '#submenu-shoes', name: 'Shoes', pathToken: 'shoes' },
                            { selector: '#submenu-leather_goods', name: 'Leather Goods', pathToken: 'leather-goods' },
                            { selector: '#submenu-ready_to_wear', name: 'Ready-to-wear', pathToken: 'ready-to-wear' },
                        ];

                        groups.forEach(group => {
                            document.querySelectorAll(group.selector + ' a.dropdown-link[href], ' + group.selector + ' a[href]').forEach(element => {
                                const anchor = element as HTMLAnchorElement;
                                const href = anchor.getAttribute('href');
                                const name = cleanName(anchor.textContent);
                                if (!href || isSkipName(name)) return;
                                addCategory('MEN - ' + group.name + ' - ' + name, href);
                            });
                        });
    ...
    async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
        ...
        while (retryAttempts < MAX_RETRY) {
            ...
            try {
                ...
                const productSelector = '.product-tile-container.product[data-pid], .product-tile-s a.tile-name[href], .product-tile-s a.d-block[href]';
                ...
                for (let i = 0; i < 20; i++) {
                    const beforeCount = await page.evaluate(() => document.querySelectorAll('.product-tile-container.product[data-pid], .product-tile-s a.tile-name[href]').length);
                    const clicked = await page.evaluate(() => {
                        const button = document.querySelector<HTMLButtonElement>('.show-more button.more[data-url]');
                        if (!button) return false;
                        const rect = button.getBoundingClientRect();
                        const style = window.getComputedStyle(button);
                        const visible = rect.width > 0 && rect.height > 0 && style.display !== 'none' && style.visibility !== 'hidden' && !button.disabled;
                    ...
                    await page.waitForFunction((count) => {
                        const button = document.querySelector<HTMLButtonElement>('.show-more button.more[data-url]');
                        const nextCount = document.querySelectorAll('.product-tile-container.product[data-pid], .product-tile-s a.tile-name[href]').length;
                        const visible = button && button.getBoundingClientRect().height > 0 && window.getComputedStyle(button).display !== 'none';
                        return nextCount > count || !visible;
                ...
                productUrls = await page.evaluate(() => {
                    ...
                    document.querySelectorAll('.product-tile-container.product[data-pid]').forEach(tile => {
                        const wrapper = tile.closest('.product-tile-s');
                        if (wrapper?.classList.contains('product-tile-xl')) return;
                        const anchor = tile.querySelector<HTMLAnchorElement>('a.tile-name[href], .product-tile-image-slide a[href], a.d-block[href]');
                        addUrl(anchor?.getAttribute('href'));
                    });

                    document.querySelectorAll('.product-tile-s:not(.product-tile-xl)').forEach(tile => {
                        const anchor = tile.querySelector<HTMLAnchorElement>('a.tile-name[href], .product-tile-image-slide a[href], a.d-block[href]');
                        addUrl(anchor?.getAttribute('href'));
                    });
        ...
        for (const [index, productUrl] of productUrls.entries()) {
            ...
            while (loadAttempts < 10) {
                try {
                    ...
                    console.log(`✅ (${index + 1}/${productUrls.length}) 벨루티 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                    await waitForBrandProductPage(page, 30000);
                    break; // ✅ 로딩 성공 시 루프 종료
                } catch (error: any) {
                    loadAttempts++;
                    console.warn(`벨루티 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                    if (loadAttempts < 3) {
    ...
    async getProductUpdate(visitUrl: string, goodsNo: string, customId: string, accountPlatform: string){
        ...
        for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
            ...
            try {
                ...
                console.warn(`🚨 berluti 업데이트 실패: ${err.message}`);
            } finally {
                if (page && !page.isClosed()) await page.close();
                if (browser) await browser.close();
            }
