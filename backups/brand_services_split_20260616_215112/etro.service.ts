        ...
        while (retryAttempts < MAX_RETRY) {
            ...
            try {
                ...
                productUrls = await page.evaluate(() => {
                    ...
                    document.querySelectorAll('article.producttile').forEach(tile => {
                        const notifyCount = Array.from(tile.querySelectorAll('.modal-sizes-cta-info'))
                            .filter(el => (el.textContent || '').toLowerCase().includes('notify me')).length;
                        const hasAvailableSize = Boolean(tile.querySelector('[data-option-available="true"], .modal-sizes-cta:not(.not-available):not([disabled])'));
                        if (notifyCount > 0 && !hasAvailableSize) return;
                        tile.querySelectorAll('a.producttile-cover[href], .producttile-name a[href], a.producttile-gallery-inner[href]').forEach(anchor => {
