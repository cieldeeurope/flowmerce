    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        try {
            ...
            const isErrorPage = await page.evaluate(() => {
                return !!document.querySelector('.error-page-container');
            });
            ...
            productDetails = await page.evaluate(() => {
                ...
                const addToCartBtn = document.querySelector('#dataiads-add-to-cart');

                // ❌ 버튼이 없거나 disabled 클래스면 즉시 품절
            ...
            console.warn(`🚨 tomford 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
