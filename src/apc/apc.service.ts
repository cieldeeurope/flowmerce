...
async getCategories(siteUrls: string[]): Promise<{ categoryName: string; url: string }[]> {
    ...
    for (const siteUrl of siteUrls) {
        ...
        while (retryCount < 3 && !success) {
            ...
            // ✅ localStorage (같이 써야 안정)
            await page.evaluateOnNewDocument(() => {
                localStorage.setItem('locale', 'nl_NL');
            ...
            try {
                ...
                const categories = await page.evaluate(() => {
                    ...
                    /* -----------------------------
                        1️⃣ details 강제 열기
                    ----------------------------- */
                    document.querySelectorAll('details.header__menu-disclosure')
                        .forEach(d => d.setAttribute('open', 'true'));

                    const mainMap = [
                        { key: 'woman', selector: 'li[data-handle="the-woman"]' },
                        { key: 'man', selector: 'li[data-handle="the-man"]' },
                    ];

                    mainMap.forEach(main => {
                        const root = document.querySelector(main.selector);
                        if (!root) return;

                        root.querySelectorAll('.mega-menu__linklist > li').forEach(li => {

                        let mid =
                            li.querySelector('a.h6')?.textContent?.trim() ||
                            li.querySelector('p.h6')?.textContent?.trim();

                        if (!mid) return;
                        mid = mid.toLowerCase();

                        // ✅ new arrivals 제거
                        if (mid.includes('new arrivals')) return;

                        li.querySelectorAll('ul li a').forEach(a => {
                            let sub = a.textContent?.trim();
                            if (!sub) return;

                            sub = sub.toLowerCase();

                            // view all 제거
                            if (sub.includes('view all')) return;
...
async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
    ...
    console.log(`아페쎄 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
        ...
        while (loadAttempts < 10) {
            try {
                ...
                console.log(`✅ (${index + 1}/${productUrls.length}) 아페쎄 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                await page.waitForSelector(
                ...
                loadAttempts++;
                console.warn(`아페쎄 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                if (loadAttempts < 3) {
        ...
        const productDetails = await page.evaluate(async () => {
            ...
            const color =
            document.querySelector('p.variant-picker__option-name')?.textContent?.trim() || '';
            ...
            try {
                ...
                if (json['@type'] === 'ProductGroup' && json.hasVariant) {
                productJson = json;
                break;
                }
            } catch {}
            ...
            .map((v: any) => {
                let size = (v.name || '').split(' - ').pop()?.trim() || '';

                if (!size) return null;

                if (size === 'TU') return '원사이즈';
            ...
            const styleId = firstVariant?.sku || '';
            const brandstyleId = styleId;

            const price = firstVariant?.offers?.price
            ? parseInt(firstVariant.offers.price, 10)
            : null;
            ...
            const block = document.querySelector(
                `.product-info__block-item[data-block-id="${id}"]`
            );

            if (!block) return;

            const texts = Array.from(
                block.querySelectorAll('.accordion__content li')
            )
                .map(li => li.textContent?.trim())
                .filter(Boolean);

            infoTexts.push(...texts);
            });

            const mainInfo = infoTexts.join('\n');

            // 🔥 madeIn 추출 (대소문자 대응 + 점 제거)
            let madeIn = '';

            const found = infoTexts.find(text =>
            ...
            .map(el => {
                ...
                // 🔥 영상 제거
                if (el.querySelector('video')) return null;
        ...
        if (productDetails?.reason === 'soldout') {
            console.warn('아페쎄 상품 품절 - 다음 productUrl로 이동');
            continue;
        }
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('아페쎄 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        try {
            ...
            const productDetails = await page.evaluate(() => {
                ...
                for (const el of scripts) {
                    try {
                    const json: any = JSON.parse(el.textContent || '{}');
                ...
                const variants: any[] = productJson.hasVariant || [];
                ...
                const firstVariant: any = validVariants[0];

                const price = firstVariant?.offers?.price
                    ? Math.floor(parseFloat(firstVariant.offers.price))
                    : 0;
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 apc 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
