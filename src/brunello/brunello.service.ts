...
async getCategories(siteUrls: string[]): Promise<{ categoryName: string; url: string }[]> {
    ...
    for (const siteUrl of siteUrls) {
      ...
      for (let attempt = 1; attempt <= 3 && !success; attempt++) {
        try {
          let categoryHtml = await getHtmlFromBrightDataUnlocker(siteUrl, {
            debugLabel: 'brunello',
          ...
          if (!categories.length && siteUrl.includes('/en-fr/')) {
            ...
            categoryHtml = await getHtmlFromBrightDataUnlocker(nlUrl, {
              debugLabel: 'brunello',
              saveRetryResponses: true,
              randomRetryDelay: true,
            });
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        try {
            ...
            const nlUrl = visitUrl.replace('/en-fr/', '/en-nl/');
            await new Promise(resolve => setTimeout(resolve, 2000));
            await page.goto(nlUrl, {waitUntil: 'domcontentloaded',timeout: 30000});
            await page.waitForSelector(
                'span.cc-price-text',
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 brunello 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
