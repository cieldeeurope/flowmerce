...
export class HermesService {
    ...
    async getCategories(siteUrls: string[]): Promise<{ categoryName: string; url: string }[]> {
        ...
        for (const pageUrl of siteUrls) {
            ...
            const html = await getHtmlFromBrightDataUnlocker(pageUrl, {
                debugLabel: 'hermes',
                ...
                try {
                    ...
                    const productHtml =
                        await getHtmlFromBrightDataUnlocker(productUrl, {
                            requiredSelector: 'script#hermes-state',
                            expect: { element: 'script#hermes-state' },
                            debugLabel: 'hermes',
                            saveRetryResponses: true,
                            randomRetryDelay: true,
                        });
                    ...
                    const price =
                        parseInt(product.price || '0', 10);

                    /* ---------------- COLOR ---------------- */

                    const color =
                        product.simpleAttributes?.colorHermes || '';
