        ...
        while (retryCount < 3 && !success) {
            ...
            try {
                ...
                const categories = await page.evaluate(() => {
                    ...
                    const topMenus = document.querySelectorAll('li.nav-item.dropdown');

                    topMenus.forEach(menu => {

                        const main = menu.querySelector(':scope > a.dropdown-link')?.textContent?.trim();
                        if (!main) return;
                        ...
                        // ❌ 제거
                        if (
                        mainLower.includes('new arrivals') ||
                        mainLower.includes('edit') ||
                        mainLower.includes('gifts')
                        ) return;

                        /* =============================
                        1️⃣ a.dropdown-link 기반 CATEGORIES
                        ============================= */
                        const categoryBlock = Array.from(
                        menu.querySelectorAll('a.dropdown-link')
                        ).find(el => el.textContent?.toLowerCase().includes('categories'));

                        if (categoryBlock) {
                        const list = categoryBlock.closest('li')?.querySelector('ul.single.inline-menu');
                        ...
                        if (list) {
                            ...
                            result.push({
                                categoryName: `${mainLower} - ${sub.toLowerCase()}`,
                                url
                            });
                            });
                        }
                        ...
                        /* =============================
                        2️⃣ p.submenu-layout-title 기반 (READY-TO-WEAR)
                        ============================= */
                        const categoryTitle = Array.from(
                        menu.querySelectorAll('p.submenu-layout-title')
                        ).find(el => el.textContent?.toLowerCase().includes('categories'));

                        if (categoryTitle) {

                        const list =
                            categoryTitle.closest('li')?.nextElementSibling?.querySelector('ul.single.inline-menu')
                            || menu.querySelector('ul.single.inline-menu');
                        ...
                        /* =============================
                        3️⃣ 단일 링크 (eyewear / beachwear)
                        ============================= */
                        menu.querySelectorAll(':scope > ul > li > a.dropdown-link[href]').forEach(a => {

                        const sub = a.textContent?.trim();
                        if (!sub) return;

                        if (sub.toLowerCase().includes('view all')) return;
                    ...
                    const unique = Array.from(
                        new Map(result.map(v => [v.url, v])).values()
                    );
...
async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
    ...
    while (retryAttempts < MAX_RETRY) {
        ...
        try {
            ...
            await page.waitForSelector(
                '.product[data-pid] .pdp-link a[href]',
                { timeout: 30000 }
            );
            ...
            await page.evaluate(() => {
                const btn = Array.from(document.querySelectorAll('button'))
                    .find(el => el.textContent?.includes('Netherlands'));
            ...
            while (true) {
                try {
                    ...
                    document.querySelectorAll('.product[data-pid]').length
                    );
                    ...
                    await page.waitForFunction(
                    (prev) =>
                        document.querySelectorAll('.product[data-pid]').length > prev,
                    { timeout: 10000 },
                    prevCount
                    ).catch(() => {});
    ...
    console.log(`끌로에 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
        ...
        while (loadAttempts < 10) {
            try {
                ...
                console.log(`✅ (${index + 1}/${productUrls.length}) 끌로에 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                await page.waitForSelector(
                ...
                loadAttempts++;
                console.warn(`끌로에 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                if (loadAttempts < 3) {
        ...
        const productDetails = await page.evaluate(() => {
            ...
            const isPreOrder = Array.from(
                document.querySelectorAll('a, button')
            ).some(el =>
                el.textContent?.toLowerCase().includes('pre-order')
            );

            const isNotify = document.querySelector('.notify-me-link');

            if (isPreOrder || isNotify) {
                return { soldOut: true };
            }
            ...
            /* -----------------------------
            1️⃣ JSON 파싱 (Product + ProductGroup)
            ----------------------------- */
            ...
            if (productJson['@type'] === 'Product') {
                ...
                styleId = productJson.sku || '';
                brandstyleId = styleId;

                price = productJson.offers?.price
                ? Math.floor(parseFloat(productJson.offers.price))
                : 0;

                color = (productJson.color || '').replace(/_/g, ' ');
                description = productJson.description || '';

                imageUrls = productJson.image || [];

                soldOut = productJson.offers?.availability?.includes('OutOfStock');
            }

            /* -----------------------------
            🔥 ProductGroup
            ----------------------------- */
            if (productJson['@type'] === 'ProductGroup') {
                title = productJson.name || '';
                styleId = productJson.sku || '';
                brandstyleId = styleId;
                ...
                const variants = productJson.hasVariant || [];

                let currentVariant =
                variants.find(v => v.sku === styleId) || variants[0];

                if (currentVariant) {
                color = (currentVariant.color || '').replace(/_/g, ' ');
                imageUrls = currentVariant.image || [];
                } else {
                imageUrls = productJson.image || [];
                }
            ...
            /* -----------------------------
            5️⃣ Product Details 추출
            ----------------------------- */
            let detailInfo = '';

            const groups = Array.from(
                document.querySelectorAll('.pdp-attr-group')
            );

            for (const group of groups) {
                const titleEl = group.querySelector('.pdp-attr-group-title span');

                if (titleEl && titleEl.textContent.trim() === 'Product Details') {
                const content = group.querySelector('.pdp-attr-group-definition');

                if (content) {
                    detailInfo = content.textContent
                    .replace(/\s+/g, ' ')
                    .trim();
                }
                break;
            ...
            if (cleanDesc && detailInfo) {
                if (detailInfo.includes(cleanDesc.slice(0, 50))) {
                mainInfo = detailInfo;
                } else {
                mainInfo = cleanDesc + '\n\n' + detailInfo;
                }
            } else {
                mainInfo = cleanDesc || detailInfo;
            }
            ...
            const sizeNodes = Array.from(
                document.querySelectorAll(
                'input[name="attribute-radio-size"][data-attr-value]'
                )
            );
            ...
            if (sizeNodes.length) {
                ...
                const value = el.getAttribute('data-attr-value') || '';

                const label = document.querySelector(`label[for="${el.id}"]`);
                const text = label?.textContent || '';

                if (text.includes('Notify me')) return;

                sizeSet.add(value.trim());
                });
        ...
        if (!productDetails || productDetails.soldOut) {
            console.warn('끌로에 상품 품절 - 다음 productUrl로 이동');
            continue;
        }
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('끌로에 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        try {
            ...
            const productDetails = await page.evaluate(() => {
                ...
                // JSON 기준
                if (productJson.offers?.availability?.includes('OutOfStock')) {
                    soldOut = true;
                }
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 chloe 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
