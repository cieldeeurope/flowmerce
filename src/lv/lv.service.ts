...
async getCategories(siteUrls: { categories1: string[]; categories2: string[] }): Promise<any[]> {
  ...
  const allUrls = [...siteUrls.categories1, ...siteUrls.categories2];

  // 🔥 핵심: URL 단위 병렬 처리
  await Promise.all(
    allUrls.map(async (siteUrl) => {
      let success = false;

      for (let attempt = 1; attempt <= 3 && !success; attempt++) {
        try {
          const categoryHtml = await getHtmlFromBrightDataUnlocker(siteUrl, {
            debugLabel: 'lv',
          ...
          if (siteUrls.categories1.includes(siteUrl)) {
          ...
          if (siteUrls.categories2.includes(siteUrl)) {
            ...
            categories = $(selector)
              .map((_, el) => {
                const href = $(el).attr('href');
                const categoryName = $(el).text().replace(/\s+/g, ' ').trim();
    ...
    while (!stop) {
      ...
      const results = await Promise.all(
        pages.map(async (page) => {
          ...
          try {
            html = await getHtmlFromBrightDataUnlocker(pageUrl, {
              debugLabel: 'lv',
              saveRetryResponses: true,
              randomRetryDelay: true,
            });
