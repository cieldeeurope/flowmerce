    ...
    //   });
      ...
      try {
        ...
        const categories = await page.evaluate(() => {
          ...
          function collectGender(rootId: string, label: string) {
            const root = document.querySelector(
              `#${rootId} ul.sub-menu__grid`,
            );
            if (!root) return;

            const EXCLUDE_SECTION = ['NEW', 'RUNWAY'];
            const EXCLUDE_CATEGORY = ['VIEW ALL'];

            const sectionMap: Record<string, string> = {};

            root
              .querySelectorAll(
                'li.show-for-large-up.sub-menu__item-level--1 h3',
              )
              .forEach(h3 => {
                const id = h3.id;
                const name = (h3 as HTMLElement).innerText
                  .trim()
                  .toUpperCase();
                sectionMap[id] = name;
              });
            ...
            root
              .querySelectorAll(
                'li.show-for-large-up.sub-menu__item-level--2 a',
              )
              .forEach(a => {
                const name = (a as HTMLElement).innerText.trim();
                const upper = name.toUpperCase();
                if (EXCLUDE_CATEGORY.includes(upper)) return;

                const describedBy = a.getAttribute('aria-describedby');
                if (!describedBy) return;

                const sectionId = describedBy.replace('a11y-comma ', '');
                const sectionName = sectionMap[sectionId];
                if (!sectionName) return;
                if (EXCLUDE_SECTION.includes(sectionName)) return;

                results.push({
                  categoryName: `${label} - ${sectionName} - ${name}`,
          ...
          function collectBags() {
            const root = document.querySelector(
              '#category-bags ul.sub-menu__grid',
            );
            if (!root) return;
            ...
            root
              .querySelectorAll('li.sub-menu__item-level--1')
              .forEach(li => {
                const h3 = li.querySelector('h3 .text-mask');
                const a = li.querySelector('a .text-mask');

                if (h3) {
                  currentSection = (h3 as HTMLElement).innerText
                    .trim()
                    .toUpperCase();
                  return;
                }

                if (!a || !currentSection) return;

                const name = (a as HTMLElement).innerText.trim();
                const upper = name.toUpperCase();

                if (currentSection === 'LINES') return;
                if (
                  upper.includes('VIEW ALL') ||
                  upper.includes('ALL BAG')
                )
                  return;

                if (currentSection === 'TYPES') {
                  results.push({
                    categoryName: `가방 - TYPES - ${name}`,
                    url: (li.querySelector('a') as HTMLAnchorElement).href,
                  });
                }
              });
          }

          collectGender('category-shop-woman', '여성');
          collectGender('category-shop-man', '남성');
          collectBags();
        ...
        /* =====================================================
         * 2️⃣ SHOES 전용 카테고리 (siteUrls2)
         * ===================================================== */
        for (const shoeUrl of siteUrls2) {
          console.log(`👟 Shoes 카테고리 진행 중: ${shoeUrl}`);
        ...
        await page.waitForSelector(
          ...
          const shoeCategories = await page.evaluate(shoeUrl => {
            const results: { categoryName: string; url: string }[] = [];

            const isWoman = shoeUrl.includes('/woman/');
            const isMan = shoeUrl.includes('/man/');
            const gender = isWoman ? '여성' : isMan ? '남성' : '';

            const baseUrl = shoeUrl.replace(/\/$/, '');

            document
              .querySelectorAll<HTMLButtonElement>(
                'button.featured-filter-button',
              )
              .forEach(btn => {
          ...
          allCategories.push(...shoeCategories);
        }
...
async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
        ...
        try {
            ...
            while (true) {
                const hasLoadMore = await page.evaluate(() => {
                    ...
                    if (!btn) return false;
                    if (btn.getAttribute('data-component-state') !== 'loaded') return false;
            ...
            const currentPageProductUrls = await page.evaluate(() => {
            return Array.from(
            ...
            productUrls.push(...currentPageProductUrls);
    ...
    console.log(`아크네 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
        ...
        while (loadAttempts < 10) {
            try {
                ...
                await page.goto(productUrl, { waitUntil: 'networkidle2', timeout: 15000 }).catch(() => null);
                 console.log(`✅ (${index + 1}/${productUrls.length}) 아크네 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                await page.waitForSelector(
                    'div.price span.sales span.value',
                ...
                loadAttempts++;
                console.warn(`아크네 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                if (loadAttempts < 3) {
        ...
        const isAvailable = await page.evaluate(() => {
            const btn = document.querySelector<HTMLButtonElement>(
                '#addToCartAndNotifyMeTrigger'
            );

            if (!btn) {
                // 버튼 자체가 없으면 안전하게 품절 처리
                return false;
            }

            const text = btn.textContent?.trim().toLowerCase() || '';

            // "notify me" 문구가 있으면 품절
            const isSoldOut = text.includes('notify me');

            return !isSoldOut; // true = 재고 있음, false = 품절
        });

        if (!isAvailable) {
        continue; // 품절이면 다음 상품으로
        }
        ...
        const productDetails = await page.evaluate(async () => {
            ...
            // 1️⃣ One size 우선
            const oneSizeEl = document.querySelector('.grid.grid--half---medium-up .color--gray');

            if (
            oneSizeEl &&
            oneSizeEl.textContent &&
            oneSizeEl.textContent.toLowerCase().includes('one size')
            ) {
            size = '원사이즈';
            } else {
            const sizeList = [];

            // 2️⃣ split size (waist) 먼저 시도
            const waistButtons = Array.from(
                document.querySelectorAll(
                'div[role="radiogroup"].variations__grid button[role="radio"][data-size-type="waist"]'
                )
            );

            if (waistButtons.length > 0) {
                waistButtons.forEach(btn => {
                if (btn.getAttribute('data-availability-site') === 'false') return;
                const text = btn.textContent?.trim();
                if (text) sizeList.push(text.replace(',', '.'));
                });
            } else {
                const defaultButtons = Array.from(
                document.querySelectorAll(
                    'div[data-component="variations"] div[role="radiogroup"] button[role="radio"]'
                )
                );
            ...
            const site = 'Acne';
            const designer = '아크네스튜디오';
            const titleElement = document.querySelector('h1[id="product-name"]');
            const title = titleElement ? titleElement.textContent?.trim() || '' : '';
            const priceElement = document.querySelector('div.price span.sales span.value');
            ...
            const breadcrumbText = Array.from(
                document.querySelectorAll('nav.breadcrumbs a.as-cell')
            )
            ...
            const imageElements = Array.from(
                ...
                'div[data-pdp-gallery-component="image"] img'
                )
            );
            ...
            let imageUrls = imageElements
                ...
                // 2️⃣ base64 placeholder 제거 → data-src
                if (url.startsWith('data:image')) {
                    url = img.getAttribute('data-src') || '';
                }

                // 3️⃣ srcset / data-srcset fallback (가장 큰 이미지)
                if (!url) {
                    const srcset =
            ...
            const isBagOrShoe = bagKeywords.some(k =>
                breadcrumbText.includes(k)
            );

            const has2021 = breadcrumbText.includes('2021');

            let reordered = [...imageUrls];
            ...
            if (isBagOrShoe) {
                ...
                if (last) reordered.unshift(last);
                imageUrls = reordered;
            } else {
                // 기본: 마지막-1 → 첫 번째
                if (reordered.length >= 4) {
                const idx = reordered.length - 2;
            ...
            const mainInfoContainer = document.querySelector(
                'div[data-description-component="content"]'
            );

            let mainInfo = '';
            let madeIn = '';     // ❗ 없을 수 있으니 기본 공란
            let styleId = '';
            ...
            // 1️⃣ 상품 설명 (p)
            const descriptionP = mainInfoContainer.querySelector(
                '#productDescriptionLongDescription'
            );

            if (descriptionP?.textContent) {
                resultLines.push(descriptionP.textContent.trim());
            }

            // 2️⃣ Product details (ul > li)
            const detailLis = mainInfoContainer.querySelectorAll(
                '#productDescriptionLongDetails li'
            );
            ...
            detailLis.forEach(li => {
                ...
                if (/^style\s*id\s*:/i.test(text)) return;
            ...
            // 3️⃣ 소재 정보 (선택)
            const materialLis = mainInfoContainer.querySelectorAll(
                '#productDescriptionFurtherInformation li'
            );
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('아크네 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        try {
            ...
            await page.goto(visitUrl, { waitUntil: 'networkidle2', timeout: 15000 }).catch(() => null);
            await page.waitForSelector(
                'div.price span.sales span.value',
                { visible: true, timeout: 30000 }
            );
            ...
            const productDetails = await page.evaluate(async () => {
                ...
                if (notifyBtn) {
                    const text = notifyBtn.textContent?.trim().toLowerCase() || '';
                ...
                // 1️⃣ One size 우선
                const oneSizeEl = document.querySelector('.grid .color--gray');
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 acne 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
