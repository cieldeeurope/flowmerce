            ...
            page.on('request', (request) => {
                ...
                try {
                    ...
                    const categories = await page.evaluate(() => {
                        ...
                        document.querySelectorAll('li[data-level="1"]').forEach(level1 => {

                            // ❌ Bestsellers 제거
                            if (level1.getAttribute('data-children') === '0') return;

                            const level1Name = level1
                            .querySelector('a[data-ref="menu-link"]')
                            ?.textContent?.trim();

                            if (!level1Name) return;

                            const level1Upper = level1Name.toUpperCase();

                            // ❌ ALL STORES 제거
                            if (level1Upper === 'ALL STORES') return;

                            level1
                            .querySelectorAll('li.menu-list__list-item.-secondary')
                            .forEach(level2 => {

                                const level2Name = level2
                                .querySelector('.ts-secondary-item')
                                ?.textContent?.trim();

                                if (!level2Name) return;

                                level2
                                .querySelectorAll('li[data-level="3"] a[data-ref="menu-link"]')
                                .forEach(a => {

                                    const level3Name = a.textContent?.trim();
                                    if (!level3Name) return;

                                    const upper = level3Name.toUpperCase();

                                    // ❌ 필터
                                    if (upper === '-') return;
                                    if (upper.includes('VIEW ALL')) return;
                                    if (upper.includes('NEW')) return;
                                    ...
                                    results.push({
                                    categoryName: `${level1Upper} - ${level2Name} - ${level3Name}`,
                                    url
                                    });
...
async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
    ...
    while (retryAttempts < MAX_RETRY) {
        ...
        try {
            ...
            await page.waitForSelector(
                'component-product-card a[href^="/products/"]',
                { timeout: 30000 }
            );

            while (true) {

                const container = await page.$('collection-load-more');

                if (!container) break;

                const isHidden = await page.evaluate(el => el.classList.contains('hidden'), container);

                // hidden이면 더 이상 페이지 없음
                if (isHidden) break;

                const btn = await container.$('button.cta-primary--load-more');
            ...
            productUrls = await page.evaluate(() => {
                ...
                const cards = document.querySelectorAll('component-product-card');
                ...
                cards.forEach(card => {
                    ...
                    'button.component-quick-add__btn[data-state="disabled"]'
                    );

                    // 품절이면 skip
                    if (disabledBtn) return;

                    const a = card.querySelector<HTMLAnchorElement>('a[href^="/products/"]');
                    ...
                    urls.push(location.origin + href);
...
console.log(`폴렌느 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
...
for (const [index, productUrl] of productUrls.entries()) {
    ...
    while (loadAttempts < 10) {
        try {
            ...
            console.log(`✅ (${index + 1}/${productUrls.length}) 폴렌느 ${category?.categoryName || '카테고리 없음'}  수집 중`);
            await page.waitForSelector(
            ...
            loadAttempts++;
            console.warn(`폴렌느 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
            if (loadAttempts < 3) {
        ...
        const productDetails = await page.evaluate(() => {
            ...
            if (productJson['@type'] === 'ProductGroup') {
                ...
                price = Math.floor(Number(available[0]?.offers?.price || 0));
                styleId = available[0]?.sku || '';
                ...
                const availability = productJson?.offers?.availability || '';

                if (availability.includes('OutOfStock')) {
                    return { reason: 'soldOut' };
                }

                price = Math.floor(Number(productJson.offers?.price || 0));
            ...
            const mainImage = productJson.image || '';
            ...
            const description = document
                .querySelector('#panel-description')
                ?.textContent
                ?.replace(/\s+/g, ' ')
                .trim() || '';

            const details = document
                .querySelector('#panel-details')
                ?.textContent
                ?.replace(/\s+/g, ' ')
                .trim() || '';

            const mainInfo = [description, details]
                .filter(Boolean)
                .join('\n');
            ...
            const galleryImgs = Array.from(
                document.querySelectorAll(
                    '.product-gallery component-zoomable-image img'
                )
            )
                .map(img => {
            ...
            const imageUrls = Array.from(new Set(galleryImgs));
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('폴렌느 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        try {
            ...
            const productDetails = await page.evaluate(() => {
                ...
                return {price,size,soldOut: false};
            });
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 polene 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
