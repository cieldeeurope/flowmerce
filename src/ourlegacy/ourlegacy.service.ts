                ...
                try {
                    ...
                    const categories = await page.evaluate(() => {
                        ...
                        if (path.startsWith('/mens')) {
                            level1 = 'MEN';
                            level2 = 'READY TO WEAR';
                        } 
                        else if (path.startsWith('/womens')) {
                            level1 = 'WOMEN';
                            level2 = 'READY TO WEAR';
                        } 
                        else if (path.startsWith('/footwear/mens')) {
                            level1 = 'FOOTWEAR';
                            level2 = 'MENS SHOES';
                        } 
                        else if (path.startsWith('/footwear/womens')) {
                            level1 = 'FOOTWEAR';
                            level2 = 'WOMENS SHOES';
                        } 
                        else if (path.startsWith('/footwear')) {
                            level1 = 'FOOTWEAR';
                            level2 = 'SHOES';
                        } 
                        ...
                        document
                        .querySelectorAll('.CiaCategoryProducts-hiddenMobile nav a[href]')
                        .forEach(a => {
...
async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
    ...
    while (retryAttempts < MAX_RETRY) {
        ...
        try {
            ...
            await page.waitForSelector(
                'article[data-product-listing-result-id]',
                { timeout: 30000 }
            );
            ...
            productUrls = await page.evaluate(() => {
                return Array.from(
                    ...
                    Array.from(
                        ...
                        'article[data-product-listing-result-id] a[href^="/"]'
                        )
                    )
...
console.log(`아워레가시 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
...
for (const [index, productUrl] of productUrls.entries()) {
    ...
    while (loadAttempts < 10) {
        try {
            ...
            console.log(`✅ (${index + 1}/${productUrls.length}) 아워레가시 ${category?.categoryName || '카테고리 없음'}  수집 중`);
            await page.waitForSelector(
                'h1.MuiTypography-subtitle3',
                { visible: true, timeout: 30000 }
            );
            break; // ✅ 로딩 성공 시 루프 종료
        } catch (error: any) {
            loadAttempts++;
            console.warn(`아워레가시 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
            if (loadAttempts < 3) {
        ...
        let categoryType = 'clothing';

        if (siteUrl.includes('/footwear') || siteUrl.includes('/accessories')) {
            categoryType = 'nonClothing';
        }


        const productDetails = await page.evaluate(async (categoryType) => {

            const site = 'Ourlegacy';
            const designer = '아워레가시';
            const titleMain =document.querySelector('h1.MuiTypography-subtitle3')?.textContent?.trim() || '';
            const titleSub = document.querySelector('.MuiTypography-body1')?.textContent?.trim() || '';
            const title = [titleMain, titleSub].filter(Boolean).join(' ');
            const priceElement = Array.from(document.querySelectorAll('p'))
            .find(p => p.textContent.includes('EUR'));
            const price = priceElement
            ? parseInt(priceElement.textContent.replace(/[^\d]/g, ''), 10)
            : null;
            const color = document.querySelector('div.css-qbbljy button')?.getAttribute('title')?.trim() || '';

            const nodes = document.querySelectorAll(
                '.css-qr0f2x button:not(.css-fh954p)'
            );
            ...
            let imageUrls = Array.from(
                document.querySelectorAll('.CiaProductHero-hiddenMobile button')
            )
            .map(btn => btn.getAttribute('value'))
            .filter((url): url is string =>
                !!url && !url.toLowerCase().endsWith('.mp4') // 🔥 핵심
            );

            // clothing이면 마지막 이미지를 맨 앞으로
            if (categoryType === 'clothing' && imageUrls.length > 1) {
                const last = imageUrls.pop();
                if (last) imageUrls.unshift(last);
            }
            ...
            // ✅ Details 전체 박스
            const detailsBox = document.querySelector('#cia-panel-details-content') as HTMLElement;

            if (detailsBox) {

                const raw = detailsBox.textContent;
            ...
            // ✅ Description 전체 박스
            const descBox = document.querySelector('#cia-panel-description-content') as HTMLElement;

            if (descBox) {

                const raw = descBox.innerText;
                ...
                descLines.forEach(line => {
                    ...
                    if (
                        ...
                        line.toLowerCase().includes('returns')
                    ) {
                        return;
                    }
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('아워레가시 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        try {
            ...
            await page.goto(visitUrl, { waitUntil: 'networkidle0', timeout: 15000 }).catch(() => null);
            await page.waitForSelector(
                'h1.MuiTypography-subtitle3',
                { visible: true, timeout: 30000 }
            );
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 ourlegacy 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
