...
export class DiorService {
  ...
  private async getHtmlFromUrlWithRetry(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        return await getHtmlFromBrightDataUnlocker(url, {
          debugLabel: 'dior',
        ...
        console.warn(
          `⚠️ Dior ${label} HTML retry ${attempt}/${maxAttempts - 1}: ${url}`,
        );
...
resolvedApiKey = cafe24Auth.accessToken;
    ...
    const productUrls = [
      ...new Set(
        [...html.matchAll(/"productLink":\{"uri":"(\/fr_fr\/fashion\/products\/[^"]+)"/g)]
          .map(match => `https://www.dior.com${match[1]}`)
      )
    ];
