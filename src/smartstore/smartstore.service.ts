  ...
  async fetchAndStoreSmartstoreCategories(
    ...
    try {
      ...
      for (const category of categories) {
        const existingCategory = await this.smartstoreCategoryRepository.findOne({
          where: {
        ...
        if (!existingCategory) {
          const newCategory = this.smartstoreCategoryRepository.create({
            categoryCode: category.id,
          ...
          await this.smartstoreCategoryRepository.save(newCategory);

          console.log(`📝 저장 완료: ${category.wholeCategoryName} (${category.id})`);
        }
      }
      ...
      return {
        message: `스마트스토어 리프 카테고리 ${categories.length}개 저장 완료`,
      };
