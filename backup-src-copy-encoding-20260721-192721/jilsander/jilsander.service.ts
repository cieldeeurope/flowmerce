...
async getCategories(siteUrls: string[], siteUrls2: string[]): Promise<{ categoryName: string; url: string }[]> {
    ...
    for (const siteUrl of siteUrls) {
        try {
            ...
            const categories = await page.evaluate(() => {
                ...
                document.querySelectorAll('li.nav-item.level-1').forEach(level1Li => {

                    const level1Btn = level1Li.querySelector('button');
                    if (!level1Btn) return;

                    const level1 = level1Btn.textContent?.trim();

                    // WOMEN / MEN만
                    if (!level1 || !['WOMEN','MEN'].includes(level1)) return;

                    level1Li.querySelectorAll('li.level-2').forEach(level2Li => {

                    const level2 = level2Li
                        .querySelector('.parent-category')
                        ?.textContent?.trim();

                    if (!level2) return;

                    // ❌ HIGHLIGHTS 제거
                    if (level2.toUpperCase().includes('HIGHLIGHTS')) return;

                    level2Li.querySelectorAll('li.level-3 a.link-level-3').forEach(a => {

                        const level3 = a.textContent?.trim();
                        if (!level3) return;

                        // ❌ View All 제거
                        if (level3.toLowerCase().includes('all')) return;
    ...
    for (const siteUrl of siteUrls2) {

        try {

        await page.goto(siteUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });

        await page.waitForSelector('.refinements-categories-link');
        ...
        const categories = await page.evaluate(() => {
            ...
            if (path.includes('/women/')) main = 'WOMEN';
            if (path.includes('/men/')) main = 'MEN';

            if (path.includes('/bags')) middle = 'BAGS';
            else if (path.includes('/shoes')) middle = 'SHOES';
            else if (path.includes('/small-leather-goods')) middle = 'SMALL LEATHER GOODS';
            else if (path.includes('/jewellery')) middle = 'JEWELLERY';
            else if (path.includes('/accessories')) middle = 'ACCESSORIES';

            document.querySelectorAll<HTMLAnchorElement>('.refinements-categories-link')
            .forEach(a => {
                ...
                results.push({
                categoryName: `${main}-${middle}-${name}`,
                url: href
                });

            });

            return Array.from(new Map(results.map(v => [v.url, v])).values());
...
async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
    ...
    while (retryAttempts < MAX_RETRY) {
        ...
        try {
            ...
            while (true) {
                ...
                const moreBtn = await page.$('.show-more a.more');
    ...
    console.log(`질샌더 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
        ...
        while (loadAttempts < 10) {
            try {
                ...
                console.log(`✅ (${index + 1}/${productUrls.length}) 질샌더 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                await page.waitForSelector(
                ...
                loadAttempts++;
                console.warn(`질샌더 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                if (loadAttempts < 3) {
        ...
        const productDetails = await page.evaluate(async () => {
            ...
            const site = 'Jilsander';
            const designer = '질샌더';
            const titleElement = document.querySelector('h1.js-product-name');
            const title = titleElement ? titleElement.textContent?.trim() || '' : '';
            const priceElement = document.querySelector('[itemprop="price"]');
            const rawPrice = priceElement?.getAttribute('content') || priceElement?.textContent || '';
            const normalizedPrice = rawPrice.replace(/[^\d,.]/g, '').replace(',', '.');
            const price = normalizedPrice ? Math.round(parseFloat(normalizedPrice)) : null;
            const color = document.querySelector('span.selected-color-container-value')?.textContent?.trim() || '';

            const seen = new Set();

            const imageUrls = Array.from(
                document.querySelectorAll('li[data-swiper-slide-index]')
            )
            .filter(li => {
                const idx = li.getAttribute('data-swiper-slide-index');
                if (!idx || seen.has(idx)) return false;
                seen.add(idx);
                return true;
            })
            .sort((a, b) =>
            parseInt(a.getAttribute('data-swiper-slide-index')) -
            parseInt(b.getAttribute('data-swiper-slide-index'))
            )
            .map(li => {
            ...
            const nodes = document.querySelectorAll(
                'ul.pdp-size-picker-list:not(.visually-hidden) button:not(.disabled)'
            );
            ...
            const items = document.querySelectorAll('#content-product-info ul.dot-list li');
            ...
            items.forEach(li => {
                ...
                // Product measures 버튼 제거
                if (li.querySelector('.size-guidelines-button')) return;

                // StyleID 처리
                if (li.id === 'copy-btn-wrapper') {
                    const id = li.querySelector('#product-mfc')?.textContent?.trim() || '';
                    styleId = id;
                    brandstyleId = id;
                ...
                if (text.toLowerCase().includes('made in')) {
                    madeIn = text.replace(/made in/i, '').trim();
                }

                mainInfo += text + '\n';
            });

            //Product measures 추가 (있을 때만)
            const measureBox = document.querySelector('#product-measures');

            if (measureBox) {

                const rows = measureBox.querySelectorAll('.single-detail-content');

                rows.forEach(row => {

                    const label = row.querySelector('.table-details-label p')?.textContent?.trim();
                    const value = row.querySelector('._measures-cm')?.textContent?.trim();

                    if (label && value) {
                    mainInfo += `${label}: ${value}\n`;
                    }
            ...
            const sizeFitList = document.querySelectorAll(
                'ul.js-size-and-fit-note-list li'
            );

            if (sizeFitList.length) {

                sizeFitList.forEach(li => {
                    const text = li.textContent?.trim();
                    if (text) {
                    mainInfo += text + '\n';
                    }
        ...
        if (productDetails?.reason === 'soldout') {
            console.warn('질샌더 상품 품절 - 다음 productUrl로 이동');
            continue;
        }
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('질샌더 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        try {
            ...
            await page.waitForFunction(() => {
                ...
                const textPrice = priceElement?.textContent?.replace(/[^\d]/g, '');
                return !!(contentPrice || textPrice);
            }, { timeout: 30000 });
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 jilsander 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
