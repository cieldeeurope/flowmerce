        ...
        while (retryCount < 3 && !success) {
            ...
            try {
                ...
                const categories = await page.evaluate(() => {
                    ...
                    const allowIds = ['Clothes', 'Bags', 'Accessories'];

                    const level1Menus = document.querySelectorAll(
                        '.nav-item.dropdown.top-menu-link'
                    );

                    level1Menus.forEach(menu => {

                        const l2Link = menu.querySelector('a.category-level-1');
                        if (!l2Link) return;

                        const level2 = l2Link.id?.trim();
                        if (!level2 || !allowIds.includes(level2)) return;

                        const dropdown = menu.querySelector('.dropdown-menu');
                        if (!dropdown) return;

                        const blocks = dropdown.querySelectorAll('.d-flex.flex-column');

                        blocks.forEach(block => {

                        const titleEl = block.querySelector('.heading-link');
                        ...
                        // ❌ 제외
                        if (title === 'ceremony' || title === 'selections') return;

                        const links = block.querySelectorAll('a.category-l3');
                        ...
                        links.forEach(a => {
                            ...
                            const key = `${level2}-${name}-${href}`;
                            if (seen.has(key)) return;
                            seen.add(key);

                            results.push({
                            categoryName: `WOMEN - ${level2} - ${name}`,
                            url: href
                            });
...
async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
    ...
    console.log(`마쥬 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
        ...
        while (loadAttempts < 10) {
            try {
                ...
                console.log(`✅ (${index + 1}/${productUrls.length}) 마쥬 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                ...
                loadAttempts++;
                console.warn(`마쥬 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                if (loadAttempts < 3) {
        ...
        if (!productDetails || productDetails.soldOut) {
            console.warn('마쥬 상품 품절 - 다음 productUrl로 이동');
            continue;
        }
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('마쥬 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 maje 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
