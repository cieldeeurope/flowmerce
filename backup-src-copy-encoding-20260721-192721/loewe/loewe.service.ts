...
const unsoldProducts = await this.productRepository.find({
    ...
    await page.authenticate({
      ...
      password: proxy.password,
        ...
        const isNotFoundPage = await page.waitForFunction(() => {
          const bodyText = document.body?.innerText || '';
          return bodyText.includes('This page cannot be found') ||
            bodyText.includes('Your search returned no results') ||
            Boolean(document.querySelector('#dy-notfound'));
        }, { timeout: 5000 }).then(() => true).catch(() => false);

        let productDetails: { price: number; size: string; soldOut: boolean } | null = null;

        if (isNotFoundPage) {
          console.warn(`로에베 상품 페이지 없음 - 품절 처리: ${visitUrl}`);
          productDetails = {
            price: 0,
            size: '',
            soldOut: true,
          ...
          productDetails = await page.evaluate(async (currentUrl) => {
          const priceElement = document.querySelector('div.css-1xdhyk6.e1laz5651');
          let price = 0;
          if (priceElement) {
              const rawPrice = priceElement.textContent?.replace(/[^\d,]/g, '') || '';
