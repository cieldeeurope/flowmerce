...
async getCategories(siteUrls: string[]): Promise<any[]> {
  ...
  private async rejectPradaCookieBanner(page: Page): Promise<void> {
    try {
      await page.waitForSelector('#__tealiumGDPRecModal .cta_reject', {
        visible: true,
        timeout: 5000,
      });
      await page.click('#__tealiumGDPRecModal .cta_reject');
    } catch {}
  }

  private async getPradaProductUrlsFromPage(page: Page): Promise<string[]> {
    return page.evaluate(() => {
      return Array.from(document.querySelectorAll<HTMLAnchorElement>('article a[href]'))
        .filter(a => {
  ...
  private async clickPradaShowMoreButton(page: Page): Promise<boolean> {
    const target = await page.evaluate(() => {
      const candidates = Array.from(
        document.querySelectorAll<HTMLElement>(
          '.ais-InfiniteHits-loadMore a, .ais-InfiniteHits-loadMore button, a[aria-label="Show More"], button[aria-label="Show More"]',
        ),
      );
      ...
      const element = candidates.find(candidate => {
        ...
        const disabled = (candidate.getAttribute('disabled') || '').trim().toLowerCase();
    ...
    const navigation = page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 5000 }).catch(() => null);
...
if (product.platform === 'smartstore' || product.platform === 'cafe24') {
  ...
  browser = await puppeteer.launch({
    ...
    while (retryAttempts < MAX_RETRY) {
        try {
            await page.goto(siteUrl, { waitUntil: 'networkidle2', timeout: 30000 });
            await this.rejectPradaCookieBanner(page);
            try {
    ...
    while (true) {
            ...
            document.querySelector('body') === null &&
              (bodyText.includes('Access Denied') ||
                  ...
                  const moreButton = Array.from(
                    document.querySelectorAll<HTMLElement>(
                      '.ais-InfiniteHits-loadMore a[aria-label], .ais-InfiniteHits-loadMore button[aria-label]'
                    )
                  ).find(element => {
                    const label = (element.getAttribute('aria-label') || '').trim().toLowerCase();
                    const disabled = (element.getAttribute('disabled') || '').trim().toLowerCase();
      ...
      if (isBlocked) {
        ...
        for (let showMoreAttempt = 0; showMoreAttempt < 20; showMoreAttempt++) {
          await this.autoScroll(page);
          (await this.getPradaProductUrlsFromPage(page)).forEach(url => collectedProductUrls.add(url));

          const beforeCount = collectedProductUrls.size;
          const moreClicked = await this.clickPradaShowMoreButton(page);
          ...
          if (collectedProductUrls.size === beforeCount) {
            break;
          }
        }

        if (!collectedProductUrls.size) {
          await this.autoScroll(page);
          (await this.getPradaProductUrlsFromPage(page)).forEach(url => collectedProductUrls.add(url));
        }
