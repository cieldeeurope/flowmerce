            ...
            while (retryCount < 3 && !success) {
                ...
                try {
                    ...
                    const categories = await page.evaluate((): { categoryName: string; url: string }[] => {
                        ...
                        const allowedTop: Record<string, string> = { women: 'Women', men: 'Men' };
                        const excludedSegments = new Set(['new-arrivals', 'collection', 'collections', 'icons', 'sale']);
                        ...
                        document.querySelectorAll('a[href]').forEach(element => {
                            ...
                            if (segments.length > topIndex + 3) return;
                            if (excludedSegments.has(middleSegment) || excludedSegments.has(leafSegment)) return;

                            const name = cleanName(anchor.textContent) || titleCaseSlug(leafSegment);
                            if (isSkipName(name)) return;

                            addCategory(allowedTop[topSegment] + ' - ' + titleCaseSlug(middleSegment) + ' - ' + name, href);
                        });
    ...
    async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
        ...
        while (retryAttempts < MAX_RETRY) {
            ...
            try {
                ...
                const productSelector = 'article.product-tile.js-product-tile, .js-grid-tile article.product-tile, a.product-image-slider-link[href], h2.product-name a[href]';
                ...
                const scrollUntilStable = async (stableDelay = 1500, stepDelay = 220) => {
                ...
                productUrls = await page.evaluate(() => {
                    ...
                    document.querySelectorAll('article.product-tile.js-product-tile, .js-grid-tile article.product-tile').forEach(tile => {
                        tile.querySelectorAll('a.product-image-slider-link[href], h2.product-name a[href], a.name-link[href]').forEach(anchor => {
        ...
        console.log(`지미추 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
        ...
        for (let index = 0; index < productUrls.length; index++) {
            ...
            const retryCurrentProduct = async (reason: string) => {
                ...
                console.warn(`지미추 상품 상세 재시도 (${retryCount}/3): ${productUrl} / ${reason}`);
                try {
                ...
                console.warn(`지미추 상품 상세 재시도 초과 - 다음 productUrl로 이동: ${productUrl}`);
                return false;
            };
            let loadAttempts = 0;
            ...
            while (loadAttempts < 10) {
                try {
                    ...
                    console.log(`✅ (${index + 1}/${productUrls.length}) 지미추 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                    ...
                    loadAttempts++;
                    console.warn(`지미추 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                    if (loadAttempts < 3) {
                        if (page && !page.isClosed()) await page.close();
                        if (browser) await browser.close();
            ...
            const titleElement = document.querySelector('.product-name-united-block .product-name, .js-product-name');
            const shortDescriptionElement = document.querySelector('.product-name-united-block .product-short-description, .product-short-description');
            const mainName = normalizeText(titleElement?.textContent);
            const shortDescription = normalizeText(shortDescriptionElement?.textContent);
            title = [mainName || title, shortDescription].filter(Boolean).join(' ');
                
            const styleId = normalizeText(productJson.productGroupID || productJson.sku || productJson.mpn || '');
            ...
            if (!productDetails || productDetails.soldOut) {
                console.warn('지미추 상품 품절 - 다음 productUrl로 이동');
                continue;
            }
            ...
            if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
                console.warn('지미추 데이터 누락 - 다음 productUrl로 이동:', productUrl);
                console.warn('지미추 누락 상세:', {
    ...
    async getProductUpdate(visitUrl: string, goodsNo: string, customId: string, accountPlatform: string){
        ...
        for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
            ...
            try {
                ...
                console.warn(`🚨 jimmychoo 업데이트 실패: ${err.message}`);
            } finally {
                if (page && !page.isClosed()) await page.close();
                if (browser) await browser.close();
            }
