...
if (product.platform === 'smartstore' || product.platform === 'cafe24') {
  ...
  private async collectBurberryProductUrlsFromListing(page: Page): Promise<string[]> {
    return await page.evaluate(async () => {
      const seen = new Set<string>();

      const normalizeProductUrl = (href: string) => {
        try {
      ...
      const collect = () => {
        const selectors = [
          'a.product-card-v2-anchor[href]',
          '[data-test="product-card-v2"] a[href]',
          'li.product-listing-shelf__product-card a[href]',
          'div.product-listing-shelf__product-card-container a[href*="-p"]',
          'a[href*="-p"]',
        ];

        document.querySelectorAll<HTMLAnchorElement>(selectors.join(', ')).forEach(anchor => {
          const url = normalizeProductUrl(anchor.getAttribute('href') || anchor.href || '');
          if (url) seen.add(url);
        });
      };
      ...
      for (let i = 0; i < 120; i++) {
        ...
        const currentY = window.scrollY || scrollingElement.scrollTop || 0;
        const atBottom = currentY + window.innerHeight >= scrollingElement.scrollHeight - 20;
        const unchanged = currentY === previousY && seen.size === previousSeenSize;

        if ((atBottom && seen.size === previousSeenSize) || unchanged) {
          stableCount++;
        } else {
          stableCount = 0;
        }
  ...
  browser = await puppeteer.launch({
    ...
    while (retryAttempts < MAX_RETRY) {
        try {
            ...
            await new Promise((resolve) => setTimeout(resolve, 2000));
            productUrls = productUrls.concat(await this.collectBurberryProductUrlsFromListing(page));
            await new Promise((resolve) => setTimeout(resolve, 2000));
            break; // 성공하면 반복 종료
        } catch (error: any) {
    ...
    while (true) {
        try {
            ...
            const moreButton = document.querySelector(
              '[aria-label="View more products"], [data-test*="view-more" i], [data-test*="load-more" i], button[class*="more" i], button[class*="load" i]',
            );
    ...
    for (const [index, productUrl] of productUrls.entries()) {
      ...
      const productDetails = await page.evaluate(async (designerFromNode) => {
        ...
        const galleryItems = document.querySelectorAll(
          'ul.gallery__gallery-items li.gallery__gallery-item, .product-details__gallery-wrapper ul.gallery__gallery-items li.gallery__gallery-item',
        );

        galleryItems.forEach(item => {
          const image = item.querySelector<HTMLImageElement>('picture img.gallery__image__source[src], img.gallery__image__source[src]');
          pushImageUrl(image?.getAttribute('src') || image?.getAttribute('data-src') || '');
        });

        if (!imageUrls.length) {
          const imageElements = document.querySelectorAll<HTMLImageElement>(
            'ul.desktop-product-gallery__gallery-items li img[src], .product-details__gallery-wrapper img.gallery__image__source[src], ul.gallery__gallery-items img.gallery__image__source[src]',
          );
          Array.from(imageElements).forEach(img => {
            pushImageUrl(img.getAttribute('src') || img.getAttribute('data-src'));
          });
        }
        ...
        let size;
        const normalizeSizeText = (value: any) =>
        ...
        const readSizeList = () => {
          ...
          return candidates
            .filter(element => {
              const label = normalizeSizeText(element.getAttribute('aria-label'));
              const text = normalizeSizeText(element.textContent);
              const value = normalizeSizeText((element as HTMLButtonElement).value);
              const merged = `${label} ${text} ${value}`;
              const disabled =
                (element as HTMLButtonElement).disabled ||
                element.getAttribute('aria-disabled') === 'true' ||
                /품절|out\s*of\s*stock|sold\s*out|unavailable/i.test(merged);
              const isControl =
                /사이즈\s*선택|사이즈\s*표|핏\s*가이드|쇼핑백|매장|배송|반품|선물|자세히|size\s*guide|fit\s*guide|add\s*to\s*bag|find\s*in\s*store/i.test(merged);
              return !disabled && !isControl;
            })
            .map(element => normalizeSizeText((element as HTMLButtonElement).value || element.textContent))
            .filter(Boolean)
            .filter((value, index, list) => list.indexOf(value) === index);
        };

        const sizePicker = document.querySelector(
          '.transactional-picker.size-picker, .size-picker.transactional-picker, [class*="size-picker"][tabindex], span[title="사이즈 선택:"]',
        ) as HTMLElement | null;
        if (!sizePicker) {
            size = '원사이즈';
          } else {
            const clickable = (sizePicker.closest('.transactional-picker, [role="button"], button, [tabindex]') || sizePicker) as HTMLElement;
            clickable.scrollIntoView({ block: 'center', inline: 'center' });
            ['pointerdown', 'mousedown', 'pointerup', 'mouseup', 'click'].forEach(type => {
              const EventConstructor = type.startsWith('pointer') && typeof PointerEvent === 'function' ? PointerEvent : MouseEvent;
              clickable.dispatchEvent(new EventConstructor(type, { bubbles: true, cancelable: true, view: window }));
            });

            let sizeList: string[] = [];
            for (let i = 0; i < 20; i++) {
              await new Promise(resolve => setTimeout(resolve, 100));
              sizeList = readSizeList();
              if (sizeList.length) break;
            }
      ...
      if (!productDetails) {
        console.warn(`버버리 데이터 누락 [productDetails] - ${productUrl}`);
      ...
      if (missingFields.length) {
        console.warn(`버버리 데이터 누락 [${missingFields.join(', ')}] - ${productUrl}`);
        continue; // 다음 productUrl로 이동
        }
