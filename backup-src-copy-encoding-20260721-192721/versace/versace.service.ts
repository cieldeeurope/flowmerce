    ...
    async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
        ...
        for (let index = 0; index < productUrls.length; index++) {
            ...
            const retryCurrentProduct = async (reason: string) => {
                ...
                console.warn(`베르사체 상품 상세 재시도 (${retryCount}/3): ${productUrl} / ${reason}`);
                try {
                ...
                console.warn(`베르사체 상품 상세 재시도 초과 - 다음 productUrl로 이동: ${productUrl}`);
                return false;
            };
            let loadAttempts = 0;
            ...
            const extractSize = () => {
                ...
                options.forEach(option => {
                    ...
                    const className = (String(element.className || '') + ' ' + String(input?.className || '')).toLowerCase();
                    const isUnavailable =
