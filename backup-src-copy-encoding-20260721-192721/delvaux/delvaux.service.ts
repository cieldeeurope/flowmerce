    ...
    async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
        ...
        while (retryAttempts < MAX_RETRY) {
            ...
            try {
                ...
                const productLinkSelector = '.product-grid-card-holder a[href], a[href*="/nl/producten/"]';

                await page.waitForSelector('.product-grid-card-holder a[href], .product-grid-card__image-holder, a[href*="/nl/producten/"]', {
                    timeout: 30000
                });
                ...
                const scrollToBottom = async () => {
                    await page.evaluate(async () => {
                        await new Promise<void>(resolve => {
                            ...
                            let lastScrollY = -1;
                            const timer = setInterval(() => {
                                const scrollStep = Math.max(650, Math.floor(window.innerHeight * 0.8));
                                window.scrollBy(0, scrollStep);
                                const currentScrollY = Math.ceil(window.scrollY + window.innerHeight);
                                const pageHeight = Math.max(document.body.scrollHeight, document.documentElement.scrollHeight);
                                const nearBottom = currentScrollY >= pageHeight - 8;

                                if (nearBottom || window.scrollY === lastScrollY) {
                                    stableTicks += 1;
                                } else {
                                    stableTicks = 0;
                                }
                ...
                const clickLoadMore = async () => {
                    return page.evaluate(() => {
                        ...
                        const candidates = Array.from(document.querySelectorAll('button, a[role="button"], .load-more__button')) as HTMLElement[];
                        ...
                        const target = candidates.find(element => {
                            ...
                            const className = normalizeText(element.className?.toString());
                            return className.includes('load-more') ||
                                /load\s*more|show\s*more|meer|toon\s*meer|laad\s*meer|voir\s*plus|afficher\s*plus/i.test(text);
                        });
                ...
                for (let round = 0; round < 30; round++) {
                    const beforeCount = await page.$$eval(productLinkSelector, elements => elements.length).catch(() => 0);

                    await scrollToBottom();
                    await new Promise(resolve => setTimeout(resolve, 2000));
                    const clicked = await clickLoadMore().catch(() => false);
                    ...
                    }, { timeout: 6000 }, productLinkSelector, beforeCount).catch(() => undefined);

                    const afterCount = await page.$$eval(productLinkSelector, elements => elements.length).catch(() => beforeCount);
                    if (!clicked && afterCount <= beforeCount) {
                        stableRounds += 1;
                    } else {
                        stableRounds = 0;
                    }
                ...
                productUrls = await page.evaluate(() => {
                    ...
                    const addUrl = (href: string | null) => {
                        if (!href || !href.includes('/producten/')) return;
                        if (href.includes('/lijst/')) return;
                        const absoluteUrl = href.startsWith('http') ? href : new URL(href, location.origin).href;
                        urls.add(absoluteUrl.split('?')[0]);
                    };

                    document.querySelectorAll('.product-grid-card-holder a[href], a[href*="/nl/producten/"]').forEach(anchor => {
        ...
        for (const [index, productUrl] of productUrls.entries()) {
            ...
            while (loadAttempts < 10) {
                try {
                    ...
                    if (!response) {
                        throw new Error('델보 page.goto 응답 없음');
                    }
                    if (response.status() >= 400) {
                        throw new Error(`델보 page.goto HTTP ${response.status()}`);
                    }
                    console.log(`✅ (${index + 1}/${productUrls.length}) 델보 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                    await page.waitForSelector('.pdp__hero-price', { visible: true, timeout: 60000 });
                    await page.waitForFunction(() => {
                        ...
                        const titleEl = document.querySelector('.pdp__hero-title');
                        const ctaEl = document.querySelector('.pdp__hero-cart-cta');
                        const priceEl = document.querySelector('.pdp__hero-price');
                        const title = normalizeText(document.querySelector('.pdp__hero-title')?.textContent);
                        const ctaText = normalizeText(document.querySelector('.pdp__hero-cart-cta')?.textContent).toLowerCase();
                        const price = normalizeText(document.querySelector('.pdp__hero-price')?.textContent);
                        const bodyText = normalizeText(document.body?.textContent);
                        const hasKnownCta = /toevoegen\s+aan\s+winkelmandje|verwittig|terug\s+op\s+voorraad|notify/i.test(ctaText);
                        const hasImage = Boolean(document.querySelector('.pdp__hero-images img[src*="delvaux-media.imgix.net/products/"], .pdp__hero-images source[srcset*="delvaux-media.imgix.net/products/"]'));
                        const hasDetails = Boolean(document.querySelector('.pdp__description, .pdp__description-listing')) || /REF\.\s*[A-Z0-9]/i.test(bodyText);
                        return Boolean(
                            title &&
                            hasKnownCta &&
                            /\d/.test(price) &&
                    ...
                    loadAttempts++;
                    console.warn(`델보 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/10): ${error.message}`);
                    if (loadAttempts < 10) {
            ...
            if (!productPageReady || !page || page.isClosed()) {
                console.warn('델보 상품 페이지 준비 실패 - 다음 productUrl로 이동');
                continue;
            }
            ...
            const productDetails = await page.evaluate(() => {
                ...
                const cleanPush = (parts: string[], text: string | null | undefined) => {
                    const value = normalizeText(text);
                    if (!value || value.length < 2) return;
                    parts.push(value);
                };

                const title = normalizeText(document.querySelector('.pdp__hero-title, h1')?.textContent);
                const price = parsePrice(document.querySelector('.pdp__hero-price')?.textContent);
                const color = normalizeText(
                    document.querySelector('.pdp__hero-product-info .product-info-row__key.--color')?.textContent ||
                    document.querySelector('.pdp__hero-product-info .color-option.active')?.getAttribute('title')
                );

                const cartText = normalizeText(document.querySelector('.pdp__hero-cart-cta')?.textContent).toLowerCase();
                const heroText = normalizeText(document.querySelector('.pdp__hero')?.textContent).toLowerCase();
                let soldOut = false;
                if (cartText) {
                    soldOut = /verwittig|terug\s+op\s+voorraad|notify|sold\s*out|out\s*of\s*stock|uitverkocht|niet\s+beschikbaar/i.test(cartText);
                    if (/toevoegen\s+aan\s+winkelmandje/i.test(cartText)) {
                        soldOut = false;
                    }
                } else {
                    soldOut = /verwittig|terug\s+op\s+voorraad|notify|sold\s*out|out\s*of\s*stock|uitverkocht|niet\s+beschikbaar/i.test(heroText);
                }

                const imageCandidates: string[] = [];
                const normalizeDelvauxImageUrl = (value: string) => {
                    try {
                        const parsed = new URL(value);
                        if (!parsed.hostname.includes('delvaux-media.imgix.net')) return value;
                        return `${parsed.origin}${parsed.pathname}?w=3072&fm=jpeg&q=100`;
                    } catch {
                        return value;
                    }
                };
                const pushImage = (value: string | null | undefined) => {
                    ...
                    if (!url || !url.includes('delvaux-media.imgix.net/products/')) return;
                    imageCandidates.push(normalizeDelvauxImageUrl(url));
                };
                ...
                document.querySelectorAll([
                    '.pdp__hero-images img',
                    '.pdp__hero-images source',
                    '.pdp__hero-slide-image img',
                    '.pdp__hero-slide-image source',
                    '.swiper-slide img[data-nuxt-pic]',
                    '.swiper-slide source[srcset*="delvaux-media.imgix.net/products/"]',
                    'img[src*="delvaux-media.imgix.net/products/"]',
                    'source[srcset*="delvaux-media.imgix.net/products/"]',
                ].join(', ')).forEach(element => {
                    pushImage(element.getAttribute('src'));
                    pushImage(element.getAttribute('data-src'));
                    pushSrcset(element.getAttribute('srcset'));
                    pushSrcset(element.getAttribute('data-srcset'));
                });

                const html = document.documentElement.innerHTML;
                const escapedMatches = (html.match(/https?:\\\/\\\/delvaux-media\.imgix\.net\\\/products\\\/[^"'<>\\\s]+/g) || []) as string[];
                escapedMatches.forEach(url => pushImage(url.replace(/\\\//g, '/')));
                const directMatches = (html.match(/https?:\/\/delvaux-media\.imgix\.net\/products\/[^"'<>\\\s]+/g) || []) as string[];
                directMatches.forEach(pushImage);
                ...
                const detailText = normalizeText(document.querySelector('.pdp__description')?.textContent);

                document.querySelectorAll('.pdp__hero-product-info .pdp__hero-product-info-row').forEach(row => {
                    const key = normalizeText(row.querySelector('.product-info-row__key')?.textContent);
                    const value = normalizeText(row.querySelector('.product-info-row__value')?.textContent);

                    if (/maat|maten/i.test(key) && value) {
                        cleanPush(mainParts, `${key}: ${value}`);
                    }
                });

                cleanPush(mainParts, document.querySelector('.pdp__description-content .ts-general-title3')?.textContent);
                cleanPush(mainParts, document.querySelector('.pdp__description-content-text')?.textContent);

                document.querySelectorAll('.pdp__description-list-item').forEach(item => {
                    const sectionTitle = normalizeText(item.querySelector('.pdp__description-list-item-title')?.textContent);
                    const lowerTitle = sectionTitle.toLowerCase();

                    if (
                        lowerTitle.includes('onderhoud') ||
                        lowerTitle.includes('verzending') ||
                        lowerTitle.includes('retourneren')
                    ) {
                        return;
                    }

                    cleanPush(mainParts, sectionTitle);
                    cleanPush(mainParts, item.querySelector('.pdp__description-list-item-content-inner')?.textContent);
                });

                const mainInfo = Array.from(new Set(mainParts)).join('\n');
                const styleMatch =
                    detailText.match(/REF\.\s*([A-Z0-9-]{8,})(?![a-z])/) ||
                    mainInfo.match(/REF\.\s*([A-Z0-9-]{8,})(?![a-z])/) ||
                    imageUrls.join(' ').match(/([A-Z]{2}\d{4}[A-Z0-9-]{6,})/);
            ...
            if (!productDetails) {
                console.warn(`델보 productDetails 생성 실패 - 다음 productUrl로 이동: ${productUrl}`);
                continue;
            }
            ...
            if (!productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
                const missingFields = [
                ...
                console.warn(`델보 데이터 누락(${missingFields.join(', ')}) - 다음 productUrl로 이동: ${productUrl}`);
                console.warn(`델보 누락 상세: title="${productDetails.title || ''}", price=${productDetails.price || 0}, styleId="${productDetails.styleId || ''}", brandstyleId="${productDetails.brandstyleId || ''}", size="${productDetails.size || ''}", imageCount=${productDetails.imageUrls?.length || 0}, mainInfoLength=${productDetails.mainInfo?.length || 0}`);
    ...
    async getProductUpdate(visitUrl: string, goodsNo: string, customId: string, accountPlatform: string){
        ...
        for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
            ...
            try {
                ...
                const response = await page.goto(visitUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
                if (!response) {
                    throw new Error('델보 업데이트 page.goto 응답 없음');
                }
                if (response.status() >= 400) {
                    throw new Error(`델보 업데이트 page.goto HTTP ${response.status()}`);
                }
                await page.waitForSelector('.pdp__hero-price', { visible: true, timeout: 60000 });
                await page.waitForFunction(() => {
                    const normalizeText = (value: string | null | undefined) => (value || '').replace(/\s+/g, ' ').trim();
