...
const PORT_POOL = [9222, 9232, 9242, 9252, 9262, 9272, 9282, 9292];
...
// // 최종 로그 파일 경로
// const errorLogPath = path.join(errorLogDir, 'lv_error_log.txt');
...
async getCategories(siteUrls: string[]): Promise<any[]> {
  ...
  const pushUnique = (items: { categoryName: string; url: string }[]) => {
  ...
  const allUrls = siteUrls;
  const MAX_CATEGORY_CONCURRENCY = 15;

  // 🔥 핵심: URL 단위 병렬 처리
  for (let i = 0; i < allUrls.length; i += MAX_CATEGORY_CONCURRENCY) {
    const chunk = allUrls.slice(i, i + MAX_CATEGORY_CONCURRENCY);

    await Promise.all(
      chunk.map(async (siteUrl) => {
      let success = false;
      ...
      for (let attempt = 1; attempt <= 3 && !success; attempt++) {
        try {
          ...
          let categories: { categoryName: string; url: string }[] = [];

          if (siteUrl === '__disabled_lv_homepage_category__') {
            ...
            categories = $('a[href]')
              .map((_, el) => {
                ...
                const absoluteUrl = new URL(href, 'https://en.louisvuitton.com')
                  .toString()
                  .split('?')[0];

                // 🔥 LV 핵심: 카테고리 URL 패턴만 허용
                // /eng-nl/women/ready-to-wear/denim/...
                if (!absoluteUrl.includes('/eng-nl/')) return null;

                // 🔥 제외할 것들
                if (
                  absoluteUrl.includes('/stories') ||
                  absoluteUrl.includes('/gifts') ||
                  absoluteUrl.includes('/new/') ||
                  absoluteUrl.includes('/perfumes') ||
                  absoluteUrl.includes('/beauty')
                ) {
                  return null;
                }
                ...
                // 구조: eng-nl / women / ready-to-wear / denim ...
                const gender = parts[1];
                const category1 = parts[2];
                const category2 = parts[3] || '';

                if (!['women', 'men'].includes(gender)) return null;
                ...
                return {
                  categoryName: `${gender} - ${category1} - ${category2}`,
                  url: absoluteUrl,
                };
              })
              .get()
          ...
          if (siteUrl !== '__disabled_lv_homepage_category__') {
            const buildCategory = (el: any) => {
              const href = $(el).attr('href');
              const rawCategoryName = $(el).text().replace(/\s+/g, ' ').trim();
              const categoryName = rawCategoryName.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
              const isImageFilter =
                $(el).find('img').length > 0 ||
                /<img\b/i.test(rawCategoryName);

              if (isImageFilter) return null;
              if (!href || !categoryName) return null;
              ...
              if (
                absoluteUrl.includes('/products/') ||
                absoluteUrl.includes('/stories') ||
                absoluteUrl.includes('/recommendations') ||
                absoluteUrl.includes('/homepage')
              ) {
                return null;
              }
              ...
              return {
                categoryName: `${parts[1] || 'Unknown'}-${parts[2] || 'Unknown'}-${categoryName}`,
                url: absoluteUrl,
              };
            };

            const navCategories = $('.lv-filters-navigation__item a.lv-filters-navigation__link[href], .lv-filters-navigation__item a[href]')
              .map((_, el) => buildCategory(el))
              .get()
              .filter(Boolean) as { categoryName: string; url: string }[];

            const selector = $('.lv-filter-text__list-item a.lv-choice[href]').length
              ? '.lv-filter-text__list-item a.lv-choice[href]'
              : 'a.lv-choice[href]';

            const filterCategories = $(selector)
              .map((_, el) => buildCategory(el))
              .get()
              .filter(Boolean) as { categoryName: string; url: string }[];
            ...
            const categoryCandidates = navCategories.length ? navCategories : filterCategories;
            categories = categoryCandidates.filter((category) => {
              const nameKey = category.categoryName.toLowerCase();
              if (localSeenNames.has(nameKey)) return false;
              if (localSeenUrls.has(category.url)) return false;
              localSeenNames.add(nameKey);
              localSeenUrls.add(category.url);
              return true;
            });
          }
          ...
          // 🔥 병렬 안전하게 결과 합치기
          pushUnique(categories);

          console.log(`LV categories collected from ${siteUrl}: ${categories.length}`);
          success = true;

        } catch (error: any) {
          console.error(`LV category fetch failed [${siteUrl}] (${attempt}/3): ${error.message}`);
        }
      }

      if (!success) {
        console.warn(`LV category fetch skipped after retries: ${siteUrl}`);
      }
      })
    );
  }
...
//       return await this.r2Service.uploadImageToR2(
//         
//         fileName,
//         imageBuffer,
//         product,
...
//       );
//     } catch (error: any) {
//       console.warn(`🚫 이미지 업로드 실패 (${imageUrl}): ${error.message}`);
//       //logErrorToDesktop(error, `오류 발생`);
//       throw error;
//     }
//   }
  ...
  return imageUrl;
    ...
    const response = await axios.get(imageUrl, {
      ...
      headers: {
        ...
        'Referer': 'https://en.louisvuitton.com/',
      },
    });
    ...
    if (
      ...
      fileName = fileName.replace(/\.(webp|avif|png)$/i, '.jpg');
    }

    // 🔥 3️⃣ R2 업로드
    const r2Url = await this.r2Service.uploadImageToR2(
...
//     const serviceType = 'lv';
//     // const proxies = loadProxies2();
...
//             // 🚨 HTML 패턴 검사 (403/Access 차단 페이지)
//             const html = await page.content();
//             if (/Page unavailable|Reference ID:|Your IP:|Date\/Time|Access Denied|Forbidden|blocked by/i.test(html)) {
//                 throw new Error("🛑 LV 차단 페이지 감지 (HTML 패턴 매칭)");
//             }

//             // 🚨 여기서 "Page not found" 감지
//             const isNotFound = await page.evaluate(() => {
//             const errorTitle = document.querySelector('.error-page__title');
//             return errorTitle?.textContent?.trim() === 'Page not found';
//             });

//             if (isNotFound) {
//             throw new Error('❌ Page not found'); // 👉 아래 catch 로 바로 넘어감
//             }

//             // 🚨 Access denied 감지 (403 Forbidden)
//             const isAccessDenied = await page.evaluate(() => {
//                 const deniedEl = document.querySelector('.lv-waiting__titles');
//                 const deniedText = deniedEl?.textContent?.trim() || '';
//                 return deniedText.includes('Access denied') || deniedText.includes('Accès refusé');
//             });

//             if (isAccessDenied) {
//             throw new Error('🛑 Access denied (403 Forbidden)'); // 👉 catch로 바로 이동
//             }
            
//             await page.reload({ waitUntil: 'domcontentloaded', timeout: 30000 });
            
//             // 1️⃣ 쿠키 팝업 (.ucm-popin-close)
//             try {
//                 await page.waitForSelector('.ucm-popin-close', {
//                     visible: true,
//                     timeout: 6000,
//                 });

//                 const popinCloseButton = await page.$('.ucm-popin-close');
//                 if (popinCloseButton) {
//                     await popinCloseButton.click();
//                     await new Promise(resolve => setTimeout(resolve, 2000));
//                     console.log('✅ 쿠키 팝업 닫힘 (.ucm-popin-close)');
//                 }
//             } catch (error: any) {
//             console.log('ℹ️ 쿠키 팝업 없음');
//             }


//             // 2️⃣ 국가 변경 모달 (.lv-modal__close)
//             try {
//                 await page.waitForSelector('.lv-modal__close', {
//                     visible: true,
//                     timeout: 8000,
//                 });

//                 const modalCloseButton = await page.$('.lv-modal__close');
//                 if (modalCloseButton) {
//                     await modalCloseButton.click();
//                     await new Promise(resolve => setTimeout(resolve, 2000));
//                     console.log('✅ 국가 변경 모달 닫힘 (.lv-modal__close)');
//                 }
//             } catch (error: any) {
//             console.log('ℹ️ 국가 변경 모달 없음');
//             }
...
//                     Referer: 'https://en.louisvuitton.com/eng-nl',
//                     Origin: 'https://en.louisvuitton.com',
//                 });
...
//                     const productUrls = Array.from(document.querySelectorAll('ul.lv-list.lv-product-list__items .lv-product-list__item div.lv-product-card__name-wrapper a[href]'))
//                     .filter(a => {
...
//         await page.evaluate(() => {
//             const buttons = Array.from(document.querySelectorAll('button.lv-paginated-list__button'));
...
//        }
//     } catch (error: any) {
//       //logErrorToDesktop(error, "카테고리 크롤링 중 에러");
...
//               }
//           }
//           //proxy = proxyManager.getNextProxy(); // 새로운 프록시 설정
//           retryAttempts++;
//           if (retryAttempts >= 30) {
//               throw new Error("30회 재시도 초과 - 크롤링 종료");
//           }
...
//     }
//     console.log(`✅ 루이비통 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
...
//             try {
//             await page.waitForSelector('.ucm-popin-close', { visible: true, timeout: 7000 });
//             const popinCloseButton = await page.$('.ucm-popin-close');
//             if (popinCloseButton) {
//                 await popinCloseButton.click();
//                 await new Promise((resolve) => setTimeout(resolve, 2000));

//                 // .lv-modal__close 버튼이 보일 때까지 기다립니다.
//                 await page.waitForSelector('.lv-modal__close', { visible: true, timeout: 8000 });
//                 await page.click('.lv-modal__close');
//                 } 
//             } catch (error: any) {
//             console.log("쿠키 없음");
//             }
...
//             await new Promise((resolve) => setTimeout(resolve, 2000));
//             console.log(`✅ (${index + 1}/${productUrls.length}) 루이비통 ${category?.categoryName || '카테고리 없음'}  수집 중`);

//             const isOutOfStock = await page.evaluate(() => {
//             const addToBagButton = document.querySelector('button.lv-product-purchase-button span.lv-loading-button__label');
//             const buttonText = addToBagButton?.textContent?.trim() || '';

//             return buttonText !== 'Add to shopping bag'; // ❗ 이외는 전부 품절
//             });
...
//               loadAttempts++;
//               //logErrorToDesktop(error, "카테고리 크롤링 중 에러");
//               console.warn(`페이지 로드 실패(재시도): ${error.message}`);
//               if (loadAttempts < 10) {

//                 //proxy = proxyManager.getNextProxy();
//                 // cookieSet = await getLouisVuittonCookies();
//                 // proxy = cookieSet.proxy;
...
//         const site = 'LV';
//         const designer = '루이비통';
//         const titleElement = document.querySelector('h1.lv-product__name.heading-s');
//         const title = titleElement ? `${titleElement?.textContent?.trim() || ''}` : '';
//         const priceElement = document.querySelector('div.lv-price.lv-product__price.body-s');
//         const price = priceElement ? parseInt(priceElement.textContent.replace(/\s/g, '').replace(/,.*€/, ''), 10) : 0;
//         const color = document.querySelector('.lv-product-variation-selector-with-preview__value')?.textContent?.trim() || '';
        
//         // ✅ LV 이미지 처리
//         const imageElements = document.querySelectorAll<HTMLImageElement>(
//         '[aria-label="Images of decorative carousel"] picture img'
//         );

//         let imageUrls: string[] = Array.from(imageElements).map((img) => {
//         const srcset: string = img.getAttribute('srcset') || img.getAttribute('data-srcset') || '';
//         if (!srcset) return ''; // srcset이 없으면 빈 문자열 반환

//         const srcList = srcset.split(',').map((item) => {
//             const parts = item.trim().split(/\s+/);
//             const url = parts[0];
//             const size = parts.length > 1 ? parseInt(parts[1], 10) || 0 : 0;
//             return { url, size };
//         });

//         const desiredSize = 1090;
//         let selectedItem = srcList.find((item) => item.size === desiredSize);
//         if (!selectedItem && srcList.length > 0) {
//             srcList.sort((a, b) => Math.abs(a.size - desiredSize) - Math.abs(b.size - desiredSize));
//             selectedItem = srcList[0];
//         }

//         let chosenUrl = selectedItem ? selectedItem.url : '';
...
//         // ✅ Front view를 대표 이미지(맨 앞)로 이동
//         const frontIndex = imageUrls.findIndex(url => url.toLowerCase().includes("front"));
//         if (frontIndex > 0) {
//         const frontImg = imageUrls.splice(frontIndex, 1)[0];
//         imageUrls.unshift(frontImg);
//         }

//         // 최종 결과
//         console.log("📸 최종 이미지 배열:", imageUrls);
...
//         let size = '';
//         // ✅ 사이즈 선택 버튼 확인
//         const sizeButton = document.querySelector('[aria-controls="lv-product-variation-selector-dropdown-list"]') as HTMLElement | null;
...
//             // ✅ 사용 가능한 사이즈만 수집 (Notify Me When Available 제거)
//             const sizeElements = document.querySelectorAll('ul.lv-product-variation-selector-dropdown__content-list li.lv-product-variation-selector-dropdown__content-item');

//             const sizeList = Array.from(sizeElements)
//                 .map(li => {
//                     const sizeSpan = li.querySelector('.lv-product-variation-selector-dropdown__label-container span.list-label-s');
//                     const notifyMe = li.querySelector('.lv-product-variation-selector-dropdown__notify-me-container');

//                     // 🚀 품절된 사이즈는 제외
//                     if (sizeSpan) {
//                         // 만약 span의 클래스에 '-unavailable'가 있다면 품절로 간주
//                         if (sizeSpan.classList.contains('-unavailable') || notifyMe) {
//                             return null; // 🚨 품절된 상품 제거
//                         }
//                         return sizeSpan.textContent?.trim();
//                     }
//                     return null;
//                 })
//                 .filter(size => size && size.trim().length > 0); // `null`과 빈 문자열 제거

//             // ✅ 모든 사이즈가 품절이라면 상품 수집 건너뛰기
//             if (sizeList.length === 0) {
//                 console.log("⚠️ 모든 사이즈가 품절입니다. 다음 상품으로 이동합니다.");
//                 return { soldOut: true,
//                     site, designer, title, price, color, mainInfo: '', madeIn: '', styleId: '', size: '', brandstyleId: '', imageUrls
//                  };
//             } else {
//                 size = sizeList.join(', '); // ✅ 콤마(,)로 연결하여 저장
//             }
//         } else {
//             // ✅ 드롭다운 없으면 리스트형 사이즈 있는지 확인
//           const sizeGroup = document.querySelector('ul.lv-list.lv-choice-group');
//           if (sizeGroup) {
//             const items = sizeGroup.querySelectorAll('li.lv-choice-group__item');

//             const rawSizes = Array.from(items).map(li => {
//               const anchor = li.querySelector('a[aria-label]');
//               const ariaLabel = anchor?.getAttribute('aria-label')?.trim() || '';
//               if (!ariaLabel.endsWith(' - Available')) return null;

//               const sizeText = li.querySelector('span.lv-product-variation-badges__item-name')?.textContent?.trim();
...
//               console.log("⚠️ 모든 사이즈가 품절입니다. 다음 상품으로 이동합니다.");
//               return {
//                 soldOut: true, site, designer, title, price, color,
//                 mainInfo: '', madeIn: '', styleId: '', size: '', brandstyleId: '', imageUrls
//               };
//             } else {
...
//         const detailsButton = document.querySelector('[aria-controls="pdp-description"]') as HTMLElement | null;
...
//         // ✅ 제품 설명
//         let mainInfo = '';
//         const descriptionElement = document.querySelector('p.lv-product__description.body-s');
//         if (descriptionElement) {
//             mainInfo += descriptionElement.textContent.trim() + '\n\n';
//         }

//         // ✅ 사이즈 정보 추가 (bdo 태그 안의 숫자만 추출)
//         const sizeElement = document.querySelector('.lv-product-dimension.body-s.lv-product-detailed-features__section');
//         if (sizeElement) {
//             const sizeNumbers = sizeElement.querySelector('bdo')?.textContent.trim() || '';
//             if (sizeNumbers) {
//                 mainInfo += `Size: ${sizeNumbers} cm\n(Length x Height x Width)\n\n`;
//             }
//         }

//         // ✅ 상세 기능 (ul > li 태그들)
//         const featureElements = document.querySelectorAll('.lv-product-detailed-features.lv-product__detailed-features .lv-product-detailed-features__description.body-s ul li');
//         let madeIn = '';

//         if (featureElements.length > 0) {
//             const featureTexts = Array.from(featureElements).map(li => li.textContent.trim());
//             mainInfo += featureTexts.join('\n') + '\n\n';

//             // 🔹 `featureElements` 에서 "Made in" 으로 시작하는 항목 찾기
//             const madeInFeature = featureTexts.find(text => text.startsWith('Made in '));
//             if (madeInFeature) {
//                 madeIn = madeInFeature;
//             }
//         }

//         // ✅ 추가적인 p 태그 정보 가져오기
//         const extraDescriptionElements = document.querySelectorAll('.lv-product-detailed-features.lv-product__detailed-features .lv-product-detailed-features__description.body-s > p');

//         if (extraDescriptionElements.length > 0) {
//             const extraText = extraDescriptionElements[0].textContent.trim(); // 첫 번째 p 태그 가져오기
//             mainInfo += extraText + '\n';

//             // 🔹 `p` 태그에서 "Made in" 추출 (없으면 featureElements에서 가져온 값 유지)
//             const madeInMatch = extraText.match(/Made in (\w+)/i);
//             if (madeInMatch) {
//                 madeIn = `Made in ${madeInMatch[1]}`;
//             }
//         }

//         // ✅ 스타일 ID (SKU 코드 추출)
//         let styleId = '';
//         const styleIdElement = document.querySelector('strong.lv-product__sku.overline');
//         if (styleIdElement) {
//             styleId = styleIdElement.textContent.trim();
//         }
...
//         // productDetails에서 soldOut 플래그가 있으면 사이즈가 모두 품절인 경우이므로 다음 productUrl로 넘어갑니다.
//         if (productDetails && productDetails.soldOut) {
//             console.warn('모든 사이즈가 품절인 상품입니다. 다음 productUrl로 이동합니다.');
//             continue;
//         }
...
//         console.warn('루이비통 데이터 누락 - 다음 productUrl로 이동');
...
//                 console.log(`✅ 상품 업데이트 완료: ${productDetails.designer} ${productDetails.title}`);
...
//                     // ★ 새로운 로직: R2에 저장된 이미지 데이터를 base64 및 원본 URL 배열로 생성
//                     const base64ImageList: string[] = [];
//                     const originThumbnailUrls: string[] = [];
...
//             //logErrorToDesktop(err, "카테고리 크롤링 중 에러");
//           }
//         //   } finally {
...
//                 LV : siteUrl,
//               }
//               await this.smartstoreApiService.deleteUnsoldSmartstoreProducts(auth,siteUrls, partnerKey, apiKey, "LV");
//           } else {
//               // ✅ 고도몰 품절 처리
...
const unsoldProducts = await this.productRepository.find({
          ...
          for (const product of unsoldProducts) {
              ...
              console.log(`루이비통 품절 처리 완료: ${product.designer} ${product.title} (styleId: ${product.styleId})`);
          }
          }
...
// const apiKey = account.apiKey;
  ...
  // },
    ...
    //             'Referer': 'https://louisvuittong.com',
    //             'Origin': 'https://louisvuittong.com',
    //             'Sec-CH-UA': '"Chromium";v="141", "Google Chrome";v="141", "Not:A-Brand";v="8"',
    //             'Sec-CH-UA-Mobile': '?0',
    ...
    //                 'ul.lv-product-variation-selector-dropdown__content-list li.lv-product-variation-selector-dropdown__content-item'
    //                 );
    ...
    //                     // 🚫 품절 사이즈 제외
    //                     if (sizeSpan && !sizeSpan.classList.contains('-unavailable') && !notifyMe) {
    //                     return sizeSpan.textContent?.trim() || null;
    //                     }
    //                     return null;
    //                 })
    //                 .filter(s => s && s.trim().length > 0);

    //                 size = sizeList.length > 0 ? sizeList.join(', ') : '';
    //             } else {
    //                 // ✅ 드롭다운이 없는 경우 → 리스트형 사이즈 확인
    //                 const sizeGroup = document.querySelector('ul.lv-list.lv-choice-group');
    //                 if (sizeGroup) {
    //                 const items = sizeGroup.querySelectorAll('li.lv-choice-group__item');
    //                 const rawSizes = Array.from(items)
    //                     .map(li => {
    //                     const anchor = li.querySelector('a[aria-label]');
    //                     const ariaLabel = anchor?.getAttribute('aria-label')?.trim() || '';
    //                     if (!ariaLabel.endsWith(' - Available')) return null;
    ...
    //                 const uniqueSizes = [...new Set(rawSizes)];
    //                 size = uniqueSizes.length > 0 ? uniqueSizes.join(', ') : '';
    //                 } else {
    //                 // ✅ 리스트 자체도 없으면 원사이즈 처리
    //                 size = '원사이즈';
    //                 }
    ...
    //             // ✅ 품절 여부 판단
    //             const soldOut = !price || !size || size.length === 0;
  ...
  // {
    ...
    //     } catch (err: any) {
    //         console.warn(`🚨 LV 업데이트 실패: ${err.message}`);
    //         if (page && !page.isClosed()) await page.close();
    //         if (browser) await browser.close();
    //     } finally {
    //         if (page && !page.isClosed()) await page.close();
...
//     const randomDelay = async (min = 3000, max = 10000) => {
...
//             const waitTime = Math.min(1000 * 2 ** retryAttempts, 5000); // 최대 5초 대기
//             await new Promise((resolve) => setTimeout(resolve, waitTime));
//         }
//     }
...
resolvedApiKey = cafe24Auth.accessToken;
      ...
      const results = await Promise.all(
        pages.map(async (page) => {
          ...
          try {
            ...
            if (errorText.includes('dead_page') || errorText.includes('404 status code') || errorText.includes('404')) {
              console.log(`LV page end (404): ${pageUrl}`);
              return {
                page,
                urls: []
              };
          ...
          const snapshotText = String(html || '').trim();
          if (snapshotText.startsWith('[') && (snapshotText.includes('"warning_code"') || snapshotText.includes('"warning"'))) {
            try {
              const snapshotResult = JSON.parse(snapshotText);
              const isDeadPage = Array.isArray(snapshotResult) && snapshotResult.some((item: any) => {
                const warningCode = String(item?.warning_code || '').toLowerCase();
                const warning = String(item?.warning || '').toLowerCase();
                return warningCode === 'dead_page' || warning.includes('404 status code') || warning.includes('dead page');
              });

              if (isDeadPage) {
                console.log(`LV page end (dead_page): ${pageUrl}`);
                return {
                  page,
                  urls: []
                };
          ...
          $('script[type="application/ld+json"]').each((_, el) => {
            try {
              ...
              if (Array.isArray(json)) {
                const itemList = json.find(
                  (x: any) => x['@type'] === 'ItemList'
                );
                if (itemList?.itemListElement) {
                  itemList.itemListElement.forEach((i: any) =>
                    urls.push(i.url)
                  );
                }
              } else if (json['@type'] === 'ItemList') {
                json.itemListElement.forEach((i: any) =>
                  urls.push(i.url)
                );
              }
          ...
          $('a[href*="/products/"]').each((_, el) => {
            ...
            try {
              const url = new URL(href, 'https://en.louisvuitton.com');
              if (!url.hostname.includes('louisvuitton.com')) return;
              url.hash = '';
              url.search = '';
              urls.push(url.href);
            } catch {
    ...
    const uniqueProductUrls =
      [...new Set(allProductUrls)];

    console.log(`루이비통 ${uniqueProductUrls.length} - ${category?.categoryName || '카테고리 없음'}`);

    if (!uniqueProductUrls.length) {
      throw new Error('LV 상품 URL 수집 실패');
    }
    ...
    for (let i = 0; i < uniqueProductUrls.length; i += chunkSize) {
      ...
      await Promise.all(
        chunk.map(async (productUrl,idx) => {
          ...
          console.log(
            `✅ (${globalIndex + 1}/${uniqueProductUrls.length}) 루이비통 ${category?.categoryName || '카테고리 없음'} 수집 중`
          );
          ...
          try {
            ...
            const rawPrice =
              $('div.lv-price.lv-product__price.body-s')
                .text()
                .replace(/\s/g, '')
                .replace(/,.*€/, '');

            const price =
              parseInt(rawPrice || '0', 10);

            /* ---------------- COLOR ---------------- */

            const color =
              $('.lv-product-variation-selector-with-preview__value')
                .text()
                .trim() || '';
            ...
            const jsonLdScript =
              $('script[type="application/ld+json"][data-hid="structured-data"]').html();

            if (jsonLdScript?.includes('OutOfStock')) {
              soldOut = true;
            }
            ...
            if (!soldOut) {
              ...
              const sizeItems =
                $('.lv-product-variation-badges__item');
              ...
              if (sizeItems.length > 0) {
                ...
                sizeItems.each((_, el) => {
                  ...
                  const isOutOfStockSize =
                    ariaLower.includes('out of stock') ||
                    ariaLower.includes('notify') ||
                    item.find('.lv-product-variation-badges__notify-me-icon.-is-out-of-stock').length > 0;

                  if (isOutOfStockSize) return;

                  const sizeName =
                    item
                      .find('.lv-product-variation-badges__item-name')
                      .text()
                      .trim() ||
                    item
                      .find('.label-s')
            ...
            const container = $('#pdp-description');
            ...
            if (container.length) {
              ...
              container
                .find('.lv-product-detailed-features__description ul li')
                .each((_, el) => {
              ...
              const madeParagraph =
                container
                  .find('.lv-product-detailed-features__description p')
                  .text()
                  .trim();
              ...
              const madeMatch =
                mainInfo.match(/made\s+in\s+([a-zA-Z]+)/i);
            ...
            $('[aria-label="Images of decorative carousel"] picture img')
              .each((_, el) => {
                ...
                let selected =
                  srcList.find(x => x.size === desiredSize) ||
                  srcList.sort((a, b) =>
                    Math.abs(a.size - desiredSize) -
                    Math.abs(b.size - desiredSize)
                  )[0];

                if (!selected) return;

                let chosenUrl = selected.url;

                if (chosenUrl.startsWith('/')) {
                  chosenUrl =
                    `https://en.louisvuitton.com${chosenUrl}`;
                }

                if (
                  chosenUrl.includes('/poster-video/') ||
                  chosenUrl.endsWith('.mp4')
                ) return;

                imageSet.add(chosenUrl);
              });

            let imageUrls = [...imageSet];

            /* ---------------- FRONT 우선 ---------------- */

            const frontIndex =
              imageUrls.findIndex(url =>
                url.toLowerCase().includes('front')
              );

            if (frontIndex > 0) {
              const front =
                imageUrls.splice(frontIndex, 1)[0];
            ...
            const styleId =
              $('strong.lv-product__sku.overline')
                .text()
                .trim() || '';
            ...
            if (productDetails.soldOut) {
              console.warn(`LV sold out product skipped: ${productDetails.title || productUrl}`);
              return;
            }
            ...
            if (productDetails.soldOut) {
              console.warn(`❌ LV 품절상품 감지 → ${productDetails.title || productUrl}`);
              return;
            }
