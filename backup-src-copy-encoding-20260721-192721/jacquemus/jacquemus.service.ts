...
async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
    ...
    const categoryHtml =
        await this.r2Service.getHtmlFromUrl2(`${siteUrl}?sz=500`);
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        try {
            ...
            const $ = cheerio.load(html || '');
            const price = parseInt($('div.product__price').first().text().replace(/[^\d]/g, ''), 10) || 0;
            const sizes = $('button.productDetail__sizesWrapper__size')
                .toArray()
                .filter(button => {
                    const btn = $(button);
                    return !((btn.attr('class') || '').includes('-disabled')) && Boolean(btn.attr('data-url'));
                })
                .map(button => {
                    const value = $(button).attr('data-value') || '';
                    return value === 'OS' ? '원사이즈' : value;
                })
                .filter(Boolean);
            const size = sizes.length ? Array.from(new Set(sizes)).join(',') : '원사이즈';
            ...
            console.log(`Jacquemus product update complete: ${product.designer} ${product.title}`);
            return;

        } catch (err: any) {
            console.warn(`jacquemus snapshot update failed (${attempt}/${MAX_RETRY}): ${err.message}`);
        }
    }
    console.error('All jacquemus snapshot update retries failed.');
}
}
