...
@Injectable()
export class IsabelmarantService {
constructor(
    ...
    for (const siteUrl of siteUrls) {
        ...
        while (retryCount < 3 && !success) {
            ...
            try {
                ...
                // ✅ en-nl 없으면 무조건 강제 삽입
                if (!currentUrl.includes('isabelmarant.com/en-nl')) {

                    console.log(`⚠️ locale 없음 → en-nl 강제 적용: ${currentUrl}`);

                    const urlObj = new URL(currentUrl);

                    const fixedUrl = `https://isabelmarant.com/en-nl${urlObj.pathname}${urlObj.search}`;
                ...
                const categories = await page.evaluate(() => {
                    ...
                    const seen = new Set<string>(); // ✅ URL 중복 방지

                    const level1 = 'WOMEN';

                    const submenus = document.querySelectorAll(
                        'u-offcanvas-submenu.offcanvas__submenu[data-name]'
                    );

                    submenus.forEach(menu => {

                        const level2 = menu.getAttribute('data-name')?.trim();
                        if (!level2) return;

                        const lowerL2 = level2.toLowerCase();

                        if (
                            ['private-sales','pre-sales','sales','private-offer','maison']
                                .includes(lowerL2)
                        ) return;
                        ...
                        links.forEach(a => {
                            ...
                            // ✅ 핵심: filter URL 제거
                            if (href.includes('filter')) return;
...
async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
    ...
    const serviceType = 'isabelmarant';
    const proxyLines = await this.r2Service.loadBrightProxies2();
    ...
    while (retryAttempts < MAX_RETRY) {
        ...
        try {
            ...
            await page.waitForSelector(
                '.product.product--teaser a[href^="/en-nl/products/"]',
                { timeout: 30000 }
            );
            ...
            productUrls = await page.evaluate(() => {
                return Array.from(
                    ...
                    Array.from(
                        ...
                        /* =========================
                            ❌ 1️⃣ In-store availability 필터
                        ========================= */
                        const hasStoreOnly = product.querySelector(
                            '.product__stores-availability'
                        );
                        if (hasStoreOnly) return null;

                        /* =========================
                            ❌ 2️⃣ JSON inventory 0 필터 (핵심)
                        ========================= */
                        const script = product.querySelector(
                            'script[data-layer-teaser-details]'
                        );

                        if (script) {
                            try {
                            const json = JSON.parse(script.textContent || '{}');
                            if (json.inventory_quantity === 0) return null;
                            } catch {}
                        }
                        ...
                        /* =========================
                            ✅ URL 추출
                        ========================= */
                        const a = product.querySelector(
                            'a[href^="/en-nl/products/"]'
                        );
    ...
    console.log(`이자벨마랑 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
        ...
        while (loadAttempts < 10) {
            try {
                ...
                console.log(`✅ (${index + 1}/${productUrls.length}) 이자벨마랑 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                await page.waitForSelector(
                ...
                loadAttempts++;
                console.warn(`이자벨마랑 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                if (loadAttempts < 3) {
        ...
        const productDetails = await page.evaluate(() => {
            ...
            const site = 'Isabelmarant';
            const designer = '이자벨마랑';
            ...
            const addToCartBtn = [...document.querySelectorAll(
                '.product__section-buttons .btn__label'
            )].find(el =>
            ...
            const priceText =
                document.querySelector(
                    '.product__sidebar .product__price-value'
                )?.textContent?.trim() || '';
            ...
            const color =
                document.querySelector('.product__section--colors strong')
                    ?.textContent
                    ?.trim() || '';
            ...
            const descAccordion = [...document.querySelectorAll(
                '.product__accordion'
            )].find(acc =>
                acc.querySelector('h2')
                    ?.textContent
                    ?.trim() === 'Description'
            );
            ...
            if (descAccordion) {
                ...
                const content = descAccordion.querySelector(
            ...
            const styleMatch = mainInfo.match(
                /Style:\s*([A-Z0-9]+(?:\s+[A-Z0-9]+)?)/i
            );

            if (styleMatch) {
                styleId = styleMatch[1].trim();
            }
            ...
            const sizeList = [
                ...document.querySelectorAll(
                    '.product__option-values li'
                )
            ]
                .filter(li =>
                    !li.classList.contains('is-unavailable')
                )
                .map(li =>
            ...
            const imageUrls = [
                ...new Set(
                    [...document.querySelectorAll(
                        '.product__gallery-media button[data-src]'
                    )]
                        .map(btn => {
        ...
        if (!productDetails || productDetails.soldOut) {
            console.warn('이자벨마랑 상품 품절 - 다음 productUrl로 이동');
            continue;
        }
        

        if (
            /ready[\s-]*to[\s-]*wear/i.test(category?.categoryName || '') &&
            Array.isArray(productDetails.imageUrls) &&
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('이자벨마랑 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        try {
            ...
            const productDetails = await page.evaluate(() => {
                ...
                🔴 Email alert → 품절
                ========================= */
                const emailBtn = document.querySelector('.product__section-buttons');
                if (emailBtn && emailBtn.textContent?.includes('Email alert')) {
                    return { soldOut: true };
                }
                ...
                if (!productJson) return { soldOut: true };
                ...
                if (!validOffers.length) return { soldOut: true };

                /* =========================
                3️⃣ price
                ========================= */
                const price = validOffers[0]?.price
                ...
                if (offerList.length > 1) {
                    ...
                    const name = o.name || '';
                    if (name.includes('/')) {
                        const extracted = name.split('/')[1]?.trim();
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 isabelmarant 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
