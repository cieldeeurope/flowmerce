    ...
    async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
        ...
        while (retryAttempts < MAX_RETRY) {
            ...
            try {
                ...
                await page.waitForSelector('product-card[data-product-url], a.util-FauxLink_Link[href], a.prd-Card_GallerySlide[href]', {
                    timeout: 30000
                });

                const viewMoreButton = await page.$('.clc-ProductGrid_LoadMoreTrigger[data-depict-collection-el="load-more"], .clc-ProductGrid_LoadMoreTrigger');
                if (viewMoreButton) {
                    const beforeCount = await page.$$eval('product-card[data-product-url], a.util-FauxLink_Link[href]', elements => elements.length).catch(() => 0);
                    await viewMoreButton.click().catch(() => null);
                    await new Promise(r => setTimeout(r, 2500));
                    await page.waitForFunction((count) => {
                        return document.querySelectorAll('product-card[data-product-url], a.util-FauxLink_Link[href]').length > count;
                    }, { timeout: 15000 }, beforeCount).catch(() => null);
                }
                ...
                productUrls = await page.evaluate(() => {
                    ...
                    const isUnavailableCard = (card: Element): boolean => {
                        const statusText = Array.from(
                            card.querySelectorAll('.prd-Card_Badge, .prd-Card_SoldOut, [data-product-card-el="variants"]')
                        )
                        ...
                        return (
                            ...
                            statusText.includes('notify me')
                        );
                    };

                    document.querySelectorAll('product-card[data-product-url]').forEach(card => {
                        if (isUnavailableCard(card)) return;

                        addUrl(card.getAttribute('data-product-url'));
                        card.querySelectorAll('a.util-FauxLink_Link[href], a.prd-Card_GallerySlide[href]').forEach(anchor => {
                            addUrl(anchor.getAttribute('href'));
                        });
                    });

                    document.querySelectorAll('a.util-FauxLink_Link[href], a.prd-Card_GallerySlide[href]').forEach(anchor => {
                        const card = anchor.closest('product-card');
                        if (card && isUnavailableCard(card)) return;
                        addUrl(anchor.getAttribute('href'));
                    });
        ...
        console.log(`토템 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
        ...
        for (const [index, productUrl] of productUrls.entries()) {
            ...
            while (loadAttempts < 10) {
                try {
                    ...
                    console.log(`✅ (${index + 1}/${productUrls.length}) 토템 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                    await page.waitForSelector(
                        '.prd-Detail_Content, .prd-Detail_Price, .prd-Media',
                        { timeout: 10000 }
                    ).catch(() => {});
                    break; // ✅ 로딩 성공 시 루프 종료
                } catch (error: any) {
                    loadAttempts++;
                    console.warn(`토템 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                    if (loadAttempts < 3) {
            ...
            const productDetails = await page.evaluate(() => {
                ...
                const title = normalizeText(document.querySelector('.prd-Detail_Title, h1')?.textContent);
                const price = parsePrice(document.querySelector('.prd-Detail_Price, [data-product-price]')?.textContent);
                const color = normalizeText(
                    document.querySelector('.prd-Colours_Colour-active')?.getAttribute('data-name') ||
                    document.querySelector('.prd-Colours_Colour-active .util-ScreenReaderOnly')?.textContent ||
                    ''
                );

                const buyText = normalizeText(document.querySelector('.prd-Detail_Buy, .prd-DetailForm')?.textContent).toLowerCase();
                let soldOut = buyText.includes('notify me when online') || buyText.includes('coming soon');

                let imageUrls = Array.from(new Set(
                    Array.from(document.querySelectorAll('.prd-Media [data-product-media-el="slide"] img, product-media img'))
                        .map(img => normalizeUrl(img.getAttribute('src')))
                        .filter(url => url.includes('toteme.com/cdn/shop/files/') || url.includes('cdn.shopify.com/s/files/'))
                ));

                if (imageUrls.length > 1) {
                    const lastImage = imageUrls[imageUrls.length - 1];
                    imageUrls = [lastImage, ...imageUrls.slice(0, -1)];
                }
                imageUrls = imageUrls.slice(0, 9);

                const sizeOptions = Array.from(document.querySelectorAll('.prd-Options_Option[data-product-options-el="option"]'));
                let size = oneSize;
                    ...
                    const sizes = Array.from(new Set(
                        sizeOptions
                            .filter(option => {
                                ...
                                const classes = Array.from(option.classList).map(className => className.toLowerCase());
                                return stock > 0 && !classes.includes('prd-options_option-disabled');
                            })
                            .map(option =>
                                normalizeText(
                                    option.querySelector('label[data-name]')?.getAttribute('data-name') ||
                                    option.querySelector('input')?.getAttribute('value') ||
                                    option.textContent
                                )
                            )
                            .filter(Boolean)
                ...
                document.querySelectorAll('.drw-ProductDetails_Accordion').forEach(section => {
                    const sectionTitle = normalizeText(section.querySelector('.drw-ProductDetails_Tab')?.textContent);
                    const lowerTitle = sectionTitle.toLowerCase();

                    if (
                        lowerTitle.includes('materials') ||
                        lowerTitle.includes('traceability') ||
                        lowerTitle.includes('returns') ||
                        lowerTitle.includes('exchanges') ||
                    ...
                    cleanPush(mainParts, section.querySelector('.drw-ProductDetails_Content')?.textContent);
                });

                cleanPush(mainParts, document.querySelector('.drw-SizeGuide_Text')?.textContent);

                const mainInfo = Array.from(new Set(mainParts)).join('\n');
                const styleMatch = mainInfo.match(/Style\s+number\s+([A-Z0-9-]+)/i);
            ...
            if (!productDetails || productDetails.soldOut) {
                console.warn('토템 상품 품절 - 다음 productUrl로 이동');
                continue;
            }
            ...
            if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
                console.warn('토템 데이터 누락 - 다음 productUrl로 이동');
    ...
    async getProductUpdate(visitUrl: string, goodsNo: string, customId: string, accountPlatform: string){
        ...
        for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
            ...
            try {
                ...
                await page.waitForSelector(
                    '.prd-Detail_Content, .prd-Detail_Price, .prd-Options',
                    { timeout: 10000 }
                ).catch(() => {});
                ...
                console.warn(`🚨 toteme 업데이트 실패: ${err.message}`);
            } finally {
                if (page && !page.isClosed()) await page.close();
                if (browser) await browser.close();
            }
