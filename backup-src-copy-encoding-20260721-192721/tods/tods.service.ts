    ...
    for (const siteUrl of urls) {
      ...
      while (retryCount < 3 && !success) {
        ...
        try {
          ...
          console.log(`🔍 ${siteUrl} 로딩 완료`);

          // header top 로 보장
          await page.evaluate(() => window.scrollTo(0, 0));
          await new Promise((resolve) => setTimeout(resolve, 500));

          // FEMME 버튼 hover + click
          await page.evaluate(() => {
            const f: any = document.querySelector('#menu_0_cta');
            if (f) {
              f.dispatchEvent(new MouseEvent('mouseover', { bubbles: true }));
              f.dispatchEvent(new MouseEvent('mouseenter', { bubbles: true }));
              if (typeof f.click === 'function') f.click();
            }
          });
          ...
          // ⭐ 패널 활성 대기
          async function waitForPanel(panelId: string) {
          ...
          const categories = await page.evaluate(async () => {
            ...
            // 패널 내부 소분류
            function collectThirdFrom(panelId: string) {
              const panel = document.querySelector(`#${panelId}`);
              if (!panel) return [];

              const liNodes = panel.querySelectorAll('.menu-3rd-level__li');

              const list: { name: string; url: string }[] = [];

              liNodes.forEach((li: any) => {
                const a = li.querySelector('a.menu-cta-itm');
                if (a && a.textContent) {
                  list.push({
            ...
            function separateLists() {
              const femme: any = document.querySelector('#menu_0_items');
              const homme: any = document.querySelector('#menu_1_items');

              const all = [
                ...Array.from(femme.querySelectorAll('.menu-2nd-level__li')),
                ...Array.from(homme.querySelectorAll('.menu-2nd-level__li'))
              ].filter((li: any) => li.querySelector('button.menu-cta-itm'));
              ...
              all.forEach((li: any) => {
                ...
                if (String(num).startsWith('1')) female.push(li);
                if (String(num).startsWith('2')) male.push(li);
              });
            ...
            // 중분류 클릭 → 소분류 생성
            async function clickAndCollect(list: any[], genderLabel: string) {
              const fullNames: { categoryName: string; url: string }[] = [];
              const genderPrefix = genderLabel === 'Femme' ? 'WOMEN' : 'MEN';

              for (const li of list) {
                const btn: any = li.querySelector('button.menu-cta-itm');
                if (!btn) continue;

                // SPAN → 텍스트
                const nameSpan: any = btn.querySelector('span.menu-cta-name');
                const midName = nameSpan?.textContent?.trim()
                ...
                const thirds = collectThirdFrom(panelId);

                thirds.forEach((third) => {
                  fullNames.push({
                    categoryName: `${genderPrefix} - ${midName} - ${third.name}`,
                    url: third.url
                  });
                });
              }
            ...
            const { female, male } = separateLists();

            const women = await clickAndCollect(female, 'Femme');

            // 남성 버튼 클릭
            const m: any = document.querySelector('#menu_1_cta');
            if (m) {
            ...
            const men = await clickAndCollect(male, 'Homme');
...
private async uploadImageToR2(page:Page, imageUrl: string, fileName: string, product: Product, partnerKey:string): Promise<string> {
  try {
    if (!imageUrl) {
      throw new Error('유효하지 않은 이미지 URL');
    }
...
if (product.platform === 'smartstore' || product.platform === 'cafe24') {
  ...
  return imageUrl;
    ...
    console.error(`이미지 업로드 실패(Puppeteer): ${error.message}`);
    //logErrorToDesktop(error, `이미지 업로드 실패`);
    throw error;
  }
}
  ...
  private async scrollUntilFullyLoaded(page: Page): Promise<void> {
    // 1) 먼저 맨 위로 이동
    await page.evaluate(() => window.scrollTo(0, 0));
    await new Promise((resolve) => setTimeout(resolve, 500)); // 약간의 대기

    // 2) 천천히 아래로 부드럽게 스크롤
    await page.evaluate(async () => {
      const distance = 300; // 한번에 이동할 px
      let totalHeight = 0;
  ...
  browser = await puppeteer.launch({
    ...
    while (retryAttempts < MAX_RETRY) {
        try {
            ...
            try {
              ...
              console.log('쿠키 배너 닫기 완료 (Continuer sans accepter)');
            } catch (e: any) {
                console.log('쿠키 배너 없음');
            }
            await new Promise((resolve) => setTimeout(resolve, 2000));

            try {
              await page.waitForSelector(
                '.changeCountryModal__actions button',
                { timeout: 5000, visible: true }
              );

              await page.click('.changeCountryModal__actions button');
              console.log('France 사이트 유지 클릭 완료');

              await new Promise(r => setTimeout(r, 2000));
            } catch {
              console.log('국가 변경 모달 없음 (France 버튼)');
            }
            ...
            try {
              while (true) {
                ...
                for (const btn of btns) {
                  ...
                  if (text && (text.includes("voir plus de résultats") || text.includes("voir plus"))) {
                    viewMoreBtn = btn;
                    break;
                  }
                }
              ...
              console.log("⚠️ TOD'S 버튼 처리 오류:", e.message);
            }
            await new Promise((resolve) => setTimeout(resolve, 2000));
        ...
        try {
            ...
            const { currentPageProductUrls, isBlocked} = await page.evaluate(() => {
                    const productUrls = Array.from(
                      document.querySelectorAll("div.card.display")
                    )
                      .filter(card => {
                        // prod-stock 이 있으면 품절 → 제외
                        const stock = card.querySelector(".prod-stock");
                        return !stock;   // stock 이 없으면 true = 재고 있음
                      })
                      .map(card => {
    ...
    console.log(`토즈 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
      ...
      while (loadAttempts < 10) {
        try {
            ...
            console.log(`✅ (${index + 1}/${productUrls.length}) 토즈 ${category?.categoryName || '카테고리 없음'}  수집 중`);
            await new Promise((resolve) => setTimeout(resolve, 2000));
      ...
      const productDetails = await page.evaluate(async () => {
        const site = 'Tods';
        const designer = '토즈';
        const titleElement = document.querySelector('h1.prod-info__name.col-8.col-md.false');
        const title = titleElement ? titleElement.textContent?.trim() || '' : '';
        const priceBox = document.querySelector('div.pdp-colors-container div.prod-price');
        let price = 0;
        if (priceBox) {
          const visiblePrice = priceBox.querySelector('small[aria-hidden="true"]');
          if (visiblePrice) {
            const raw = visiblePrice.textContent.replace(/[^\d]/g, '');
            price = parseInt(raw, 10);
          }
        }

        let color = '';
        // ⬆️ Case 1 — <div id="pdp-collapse-clr"> 타입
        const collapseClr = document.querySelector('#pdp-collapse-clr span');
        if (collapseClr) {
          color = collapseClr.textContent
          .replace(/,/g, '')
          .replace(/\s+/g, ' ')
          .trim();
        }

        // ⬆️ Case 2 — <div class="pdp-colors-container"> 타입
        if (!color) {
          const fixedClr = document.querySelector('.pdp-colors-container .prod-clr span');
          if (fixedClr) {
            color = fixedClr.textContent
            .replace(/,/g, '')
            .replace(/\s+/g, ' ')
            .trim();
          }
        ...
        const pics = document.querySelectorAll(
          "ul.prod-glly__row li.prod-glly__c-img picture"
        );

        const imageUrls = Array.from(pics)
          .map((picture) => {
            ...
            let url = srcset.split(",")[0].trim().split(" ")[0];
        ...
        if (!sizeElements || sizeElements.length === 0) {
          ...
          const sizeList = Array.from(sizeElements)
            ...
            .filter(li => {
              ...
              const isOffCta = li.classList.contains('size-itm--offCta');
              const hasNotify = li.querySelector('.size-itm__info a');
              return ariaDisabled !== 'true' && !isOffCta && !hasNotify;
            })
            // ✅ 사이즈 텍스트 추출
            .map(li => {
              const span = li.querySelector('.size-itm__val');
              if (!span) return null;

              let text = span.textContent.trim();

              // 🔥 소수점 콤마만 점으로 변환 (35,5 → 35.5)
          ...
          // ⭐ 원사이즈 판별
          const isOneSize = sizeList.some(s => {
            const u = s.toUpperCase();
            return (
              u.includes('TAILLE UNIQUE') ||
              u.includes('ONE SIZE') ||
              u === 'UNI'
            );
          });
        ...
        const box = document.querySelector(".prod-dscr");
        ...
        if (box) {
          mainInfo = (box as HTMLElement).innerText.trim();

          const madeInMatch = mainInfo.match(/Fabriqué en\s+([A-Za-zÀ-ÿ]+)/i);
          if (madeInMatch) {
            madeIn = madeInMatch[1].trim();
          }

          const styleSpan = box.querySelector("span.paragraph--sm");
          if (styleSpan) {
            const txt = styleSpan.textContent.trim();
            const codeMatch = txt.match(/MOD\.\s*(.+)/i);
            if (codeMatch) styleId = codeMatch[1].trim();
          }
      ...
      if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
        console.warn('토즈 데이터 누락 - 다음 productUrl로 이동');
...
const unsoldProducts = await this.productRepository.find({
          ...
          for (const product of unsoldProducts) {
              ...
              console.log(`토즈 품절 처리 완료: ${product.title} (styleId: ${product.styleId})`);
          }
          }
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        for (let settleAttempt = 0; settleAttempt < 3; settleAttempt++) {
        ...
        await page.waitForFunction(() => document.readyState !== 'loading', { timeout: 10000 }).catch(() => null);
        await page.waitForSelector(
          'div.pdp-colors-container div.prod-price, ul.pdp-sizes li.size-itm, body',
          { timeout: 15000 },
        ).catch(() => null);

        let productDetails: { price: number; size: string; soldOut: boolean } | null = null;

        for (let evaluateAttempt = 1; evaluateAttempt <= 3; evaluateAttempt++) {
          try {
          ...
          return { price, size ,soldOut};
            ...
            const message = evaluateError?.message || '';
            const isNavigationContextError =
              message.includes('Execution context was destroyed') ||
              message.includes('Cannot find context with specified id') ||
              message.includes('Most likely the page has been closed');

            if (!isNavigationContextError || evaluateAttempt >= 3) {
              throw evaluateError;
            }

            console.warn(`Tods DOM read retried after navigation (${evaluateAttempt}/3): ${message}`);
        ...
        console.warn(`🚨 Tods 업데이트 실패: ${err.message}`);
