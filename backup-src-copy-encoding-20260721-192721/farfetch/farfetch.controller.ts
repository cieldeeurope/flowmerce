...
const currentCategoryCode = godoMallCategoryCode[i];
        ...
        await this.farfetchService.getProductsFromCategory(currentUrl, partnerKey, apiKey, customId, accountPlatform, currentCategoryCode);
      ...
      return { message: `모든 Farhfetch 카테고리 수집이 성공적으로 완료되었습니다.` };
    } catch (error: any) {
      console.error(`Farhfetch ${currentUrl} 수집 중 오류 발생:`, error);
      throw error;
    }
  }
}
