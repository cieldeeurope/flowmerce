...
export class RogervivierService {
    ...
    private parseRogerVivierInitialProductData(html: string): any {
        const $ = cheerio.load(html || '');
        const scripts = $('script')
            .map((_, script) => this.normalizeText($(script).html()))
            .get()
            .filter(script => script.startsWith('{"props"') && script.includes('"initialState"'));

        for (const script of scripts) {
            try {
                const parsed = JSON.parse(script);
                const productData = parsed?.props?.initialState?.product?.data;
    ...
    private extractRogerVivierSizesFromProductData(productData: any, oneSize: string): {
        sizes: string[];
        rawSizeItems: any[];
        sizeType: string;
    } {
        const sizeType = this.normalizeText(productData?.size?.sizeType);
        const variants = Array.isArray(productData?.size?.variants)
            ? productData.size.variants
            : [];

        const rawSizeItems = variants.map((variant: any) => ({
            text: this.normalizeText(variant?.size),
            sizeCode: this.normalizeText(variant?.sizeCode),
            sku: this.normalizeText(variant?.sku),
            stockLevel: variant?.stockLevel,
            stockLevelStatus: this.normalizeText(variant?.stockLevelStatus),
            messageStock: this.normalizeText(variant?.messageStock),
            source: 'initialState.product.data.size.variants',
        }));

        const sizes: string[] = Array.from(new Set<string>(
            variants
                .filter((variant: any) => {
                    const statusText = this.normalizeForMatch([
                        variant?.stockLevelStatus,
                        variant?.messageStock,
                    ].filter(Boolean).join(' '));
                    const stockLevel = Number(variant?.stockLevel);
                    const hasPositiveStock = Number.isFinite(stockLevel) && stockLevel > 0;
                    const hasAvailableText =
                        statusText.includes('instock') ||
                        statusText.includes('lowstock') ||
                        statusText.includes('disponible') ||
                        statusText.includes('available');
                    const isOutOfStock =
                        statusText.includes('outofstock') ||
                        statusText.includes('out of stock') ||
                        statusText.includes('soldout') ||
                        statusText.includes('sold out') ||
                        statusText.includes('epuise') ||
                        statusText.includes('me prevenir');

                    return !isOutOfStock && (hasPositiveStock || hasAvailableText);
                })
                .map((variant: any) => {
                    const sizeText = this.normalizeText(variant?.size);
                    if (sizeType === 'one_size' || this.normalizeForMatch(sizeText).includes('taille unique')) {
                        return oneSize;
                    }
                    return sizeText || this.normalizeText(variant?.sizeCode);
                })
    ...
    private async getRogerVivierProductHtml(url: string): Promise<string> {
        return this.getRogerVivierHtml(url, {
    ...
    private parseRogerVivierProductDetails(html: string, productUrl: string): any {
        ...
        let rawSizeItems = $('#size-selector .selectSize-item-container')
            .map((_, item) => {
                const element = $(item);
                return {
        ...
        let sizeSource = sizes.length ? 'dom-size-selector' : '';

        if (!sizes.length) {
            const initialProductData = this.parseRogerVivierInitialProductData(html);
            const nextSizeData = this.extractRogerVivierSizesFromProductData(initialProductData, oneSize);
            if (nextSizeData.rawSizeItems.length) {
                rawSizeItems = nextSizeData.rawSizeItems;
                sizeSource = 'initialState.product.data.size.variants';
            }
            if (nextSizeData.sizes.length) {
                sizes = nextSizeData.sizes;
            }
        }
    ...
    async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
        ...
        const processProduct = async (productUrl: string, index: number) => {
            ...
            try {
                const productHtml = await this.getRogerVivierProductHtml(productUrl);
...
const unsoldProducts = await this.productRepository.find({
    ...
    async getProductUpdate(visitUrl: string, goodsNo: string, customId: string, accountPlatform: string){
        ...
        for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
            try {
                const productHtml = await this.getRogerVivierProductHtml(visitUrl);
