...
function pickProxy(proxyLines: string[]) {
  ...
  @Injectable()
  export class ThombrowneService {
    constructor(
            ...
            page.on('request', (request) => {
                ...
                try {
                    ...
                    if (currentUrl.includes('/ko-kr')) {
                        const enEuUrl = currentUrl.replace('/ko-kr', '/en-eu');

                        console.log(`🔁 locale 변경 재접속: ${enEuUrl}`);

                        await page.goto(enEuUrl, {
                    ...
                    const categories = await page.evaluate(() => {
                        ...
                        const TARGETS = [
                        { selector: '[data-desktop-group="men"]' },
                        { selector: '[data-desktop-group="women"]' },
                        { selector: '[data-desktop-group="children"]' },
                        ];

                        // ❌ 제외 키워드
                        const EXCLUDE_REGEX = /\b(view\s*all|new|highlights|collection|collections)\b/i;
                        ...
                        root
                            .querySelectorAll('li.border-b.border-light-grey.leading-7 a[href]')
                            .forEach(a => {
                            ...
                            if (!name) return;
                            if (EXCLUDE_REGEX.test(name)) return;
        ...
        try {
            ...
            productUrls = Array.from(
                new Set(
                    await page.evaluate(() => {
                    return Array.from
                    (document.querySelectorAll<HTMLAnchorElement>('a.product-card--second'))
                    .map(a => a.href);
                    })
                )
            );
...
console.log(`톰브라운 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    while (loadAttempts < 10) {
        try {
            ...
            if (/\/ko-kr(\/|$)/.test(currentUrl)) {
             ...
             console.log(`✅ (${index + 1}/${productUrls.length}) 톰브라운 ${category?.categoryName || '카테고리 없음'}  수집 중`);
            await page.waitForSelector(
            ...
            loadAttempts++;
            console.warn(`톰브라운 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
            if (loadAttempts < 3) {
        ...
        const isAvailable = await page.evaluate(() => {
            const btn = document.querySelector(
                'button.mt-2.button.button--primary.w-full'
            );

            if (!btn) return true; // 버튼 자체가 없으면 구매 가능으로 간주

            const text = btn.textContent?.trim() || '';
            return !/notify me when available/i.test(text);
        });
        ...
        const productDetails = await page.evaluate(async (designerFromNode) => {
            ...
            const sizeElements = Array.from(
                document.querySelectorAll(
                    'div.main-product li.relative.group [data-available="true"]'
                )
            );
            ...
            // ✅ 원사이즈 단일
            if (sizes.length === 1 && sizes[0] === '원사이즈') {
                size = '원사이즈';
            } else {
            ...
            const site = 'Thombrowne';
            const designer = designerFromNode;
            const titleElement = document.querySelector('h2.main-product--title');
            const title = titleElement ? titleElement.textContent?.trim() || '' : '';
            const priceElement = document.querySelector('div.main-product--price');
            let price = 0;
            if (priceElement) {
                const text = Array.from(priceElement.childNodes)
                    .filter(node => node.nodeType === Node.TEXT_NODE)
                    .map(node => node.textContent)
                    .join('')
                    .trim();
                price = parseInt(text.replace(/[^\d]/g, ''), 10);
            }
            const color = document.querySelector('p.m-selector__title')?.textContent?.trim() || '';

            
            let imageUrls = Array.from(
                document.querySelectorAll('.main-product--images picture')
            )
                .map(picture => {
                // 1️⃣ 1280px source 우선
                const source1280 = picture.querySelector(
                    'source[media="(min-width:1280px)"]'
                ) as HTMLSourceElement | null;

                let url = '';

                if (source1280 && source1280.srcset) {
                    url = source1280.srcset;
                } else {
                    // 2️⃣ fallback img
            ...
            '.product-accordion-content.product-description'
            );
            ...
            // 🔹 전체 텍스트를 p 단위로 수집
            const ps = Array.from(mainInfoContainer.querySelectorAll('p'));
            ...
            // 🔹 p 태그 밖에 단독으로 있는 상품코드 추출
            const rawText = mainInfoContainer.textContent || '';

            if (!styleId && mainInfoContainer) {
                const textNodes = Array.from(mainInfoContainer.childNodes)
                    .filter(node => node.nodeType === Node.TEXT_NODE)
                ...
                const match = combinedText.match(/\b([A-Z0-9]{6,}-[A-Z0-9\-]+)\b/);
                if (match) {
                    styleId = match[1];
                }
            }
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('톰브라운 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        try {
            ...
            let navigationResponse = await page.goto(visitUrl, {
                waitUntil: 'domcontentloaded',
                timeout: 20000,
            }).catch(() => null);
            ...
            if (/\/ko-kr(\/|$)/.test(currentUrl)) {
                ...
                navigationResponse = await page.goto(enEuUrl, {
                    waitUntil: 'domcontentloaded',
                    timeout: 20000,
                }).catch(() => null);
                await new Promise(resolve => setTimeout(resolve, 1200));
            ...
            const getProductHandle = (rawUrl: string) => {
                try {
                    return new URL(rawUrl).pathname.match(/\/products\/([^/?#]+)/)?.[1] || '';
                } catch {
                    return '';
                }
            };
            const expectedProductHandle = getProductHandle(visitUrl);
            const currentProductHandle = getProductHandle(currentUrl);
            const redirectedAwayFromProduct =
                Boolean(expectedProductHandle) &&
                currentProductHandle !== expectedProductHandle;
            ...
            if (navigationStatus === 404 || is404 || redirectedAwayFromProduct) {
                if (redirectedAwayFromProduct) {
                    console.warn(`thombrowne product redirected away, soldOut: ${visitUrl} -> ${currentUrl}`);
                }
                productSoldOut = true;
            } else {
                await page.waitForSelector(
            ...
            const productDetails = await page.evaluate(async (productSoldOut) => {
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 thombrowne 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
