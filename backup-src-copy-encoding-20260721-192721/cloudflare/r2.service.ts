...
export class R2Service {
  ...
  private readonly DATASET_ID =
    ...
    process.env.BRIGHTDATA_SNAPSHOT_DATASET_ID;
  ...
  private getOfficialStyleId(product: Product): string {
    return String(product.styleId || '').replace(/\s+/g, ' ').trim();
  }

  private getOfficialBrandStyleId(product: Product): string {
    return String(product.brandstyleId || '').replace(/\s+/g, ' ').trim();
  }

  private getOfficialGoodsCd(product: Product): string {
    const site = (product.site || 'unknown').toLowerCase();
    const prefix = site === 'farfetch' || site === 'cettire'
      ? site
      : `official_${site}`;

    return `${prefix}_${this.getOfficialStyleId(product)}`;
  }
...
async getHtmlFromUrl2(url: string): Promise<string> {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    try {
      ...
      const deadPage = Array.isArray(data) && data.some((item: any) => {
      ...
      if (deadPage) {
        throw new Error(`dead_page 404 status code: ${url}`);
      }
      ...
      if (errorText.includes('dead_page') || errorText.includes('404 status code')) {
        throw error;
      }
      console.warn(`⚠️ Snapshot 실패 → retry (${attempt}) → ${url} - ${error?.message || error}`);

    }

  }

  throw new Error(`❌ HTML retry 실패 → ${url}${lastError ? ` - ${lastError?.message || lastError}` : ''}`);
}

  async getTextFromUrl2(url: string): Promise<string> {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    try {
      ...
      const text = this.getTextFromSnapshot(data);

      if (!text) {
        console.warn(`⚠️ Snapshot text 없음 → retry (${attempt}) → ${url}`);
        continue;
      }
      ...
      console.warn(`⚠️ Snapshot text 실패 → retry (${attempt}) → ${url} - ${error?.message || error}`);

    }

  }

  throw new Error(`❌ Snapshot text retry 실패 → ${url}${lastError ? ` - ${lastError?.message || lastError}` : ''}`);
}
  ...
  private getBrightDataTriggerEndpoint(): string {
    if (!this.API_KEY) {
      throw new Error('BRIGHTDATA_API_KEY 또는 BRIGHTDATA_API_TOKEN 이 없습니다.');
    }

    if (!this.DATASET_ID) {
      throw new Error('BRIGHTDATA_DATASET_ID 또는 BRIGHTDATA_CRAWL_DATASET_ID 또는 BRIGHTDATA_SNAPSHOT_DATASET_ID 가 없습니다.');
    }

    return `https://api.brightdata.com/datasets/v3/trigger?dataset_id=${encodeURIComponent(this.DATASET_ID)}&include_errors=true`;
  }

  private getBrightDataErrorMessage(err: any): string {
    const status = err?.response?.status;
    const data = err?.response?.data;
    const dataText = typeof data === 'string' ? data : data ? JSON.stringify(data) : '';
    return [status ? `status=${status}` : '', err?.message || '', dataText]
      .filter(Boolean)
      .join(' / ');
  }
    ...
    while (retry < 5) {
      try {
        ...
        const res = await axios.post(
          this.getBrightDataTriggerEndpoint(),
          data,
          {
            headers: {
        ...
        if (!snapshotId) {
          throw new Error(`snapshot_id missing: ${JSON.stringify(res.data)}`);
        }
        ...
        const status = err?.response?.status;
        const msg = this.getBrightDataErrorMessage(err);
        ...
        if (isNetworkError) {
          retry++;
          console.warn(`⚠️ trigger retry (${retry}) → ${url} - ${msg}`);

          await this.sleep(Math.min(15000, 3000 * retry)); // 점점 늘림
          continue;
        }

        throw new Error(`Bright Data snapshot trigger failed: ${msg}`);
      }
    }
  ...
  private getHtmlFromSnapshot(data: any): string | null {
    ...
    if (first.page_html && typeof first.page_html === 'string') {
      ...
      if (first.ld_json) {
        ...
        if (!html.includes('id="brightdata-ld-json"')) {
          html = html.includes('</body>')
            ? html.replace('</body>', `${ldJsonScript}</body>`)
            : `${html}${ldJsonScript}`;
        }
      }
  ...
  private getTextFromSnapshot(data: any): string | null {
    if (!Array.isArray(data) || data.length === 0) return null;
    ...
    for (const field of stringFields) {
      ...
      if (value && typeof value === 'object' && (value.stats || value.products || value.hitsAlgolia)) {
        return JSON.stringify(value);
      }
    }

    if (first && typeof first === 'object' && (first.stats || first.products || first.hitsAlgolia)) {
...
async loadBrightProxies2(): Promise<string[]> {
  ...
  createContent = (
    ...
    const joinWithoutDuplicateId = (
      parts: Array<string | null | undefined>,
      uniqueId?: string | null,
    ) => {
      const normalizedId = normalizeToken(uniqueId);
      let hasId = false;
      ...
      return parts
        ...
        .filter(part => {
          ...
          if (!normalizedId || !normalizedPart.includes(normalizedId)) {
            return true;
          }
    ...
    if (['Farfetch', 'Cettire'].includes(product.site)) {
      productTitle = joinWithoutDuplicateId(
        [product.designer, product.title, product.brandstyleId || ''],
      ...
      if (product.site === 'Dior') {
        productTitle = product.title.includes(product.designer)
          ? joinWithoutDuplicateId([product.title, product.styleId], product.styleId)
          : joinWithoutDuplicateId([product.designer, product.title, product.styleId], product.styleId);
...
async uploadXmlToR2Farfetch(
  ...
  <data>
    <goods_data>
      ...
      <makerNm>${product.designer}</makerNm>
      <goodsModelNo>${this.getOfficialBrandStyleId(product)}</goodsModelNo>
      <magnifyImageData idx="1"><![CDATA[${product.mainImageUrl}]]></magnifyImageData> <!-- 대표 이미지 -->
      ${product.additionalImageUrls.map((url,index) => `
        <magnifyImageData idx="${index + 2}"><![CDATA[${url}]]></magnifyImageData>
      `).join('')}
...
async uploadXmlToR2Cettire(
  ...
  async uploadXmlToR2(
    ...
    const getGoodsCd = (product: Product): string => this.getOfficialGoodsCd(product);
  ...
  <data>
    <goods_data>
      ...
      <goodsSearchWord>${product.title.split(' ').join(',')}</goodsSearchWord>
      <goodsPrice>${price}</goodsPrice>
      <goodsCd>${escapeXml(getGoodsCd(product))}</goodsCd>
      <makerNm>${escapeXml(product.designer)}</makerNm>
      <goodsModelNo>${escapeXml(this.getOfficialBrandStyleId(product))}</goodsModelNo>
      ${imageData}
    ...
    * =============================== */
    ...
    <data>
      <goods_data>
        ${product.goodsno ? `<goodsNo>${product.goodsno}</goodsNo>` : ''}  <!-- 상품 수정 시 goodsno 포함 -->
        <goodsNmFl>d</goodsNmFl>
        <goodsNm>${escapeXml(product.title)}</goodsNm>
        <goodsSearchWord>${escapeXml(product.title.split(' ').join(','))}</goodsSearchWord>
        ...
        <naverProductFlag>b</naverProductFlag>
        <naverAgeGroup>a</naverAgeGroup>
        <naverTag>${escapeXml(`${product.designer} ${product.title} ${product.brandstyleId}`.split(' ').join('|'))}</naverTag>
  ...
  async uploadXmlToR2Update(
    ...
    try {
      ...
      if (!soldOut) {
        try {
          product.price = updateDisplayPrice;
      ...
      if (!soldOut && typeof updateDisplayPrice === 'number' && Number.isFinite(updateDisplayPrice)) {
        (product as any).updateDisplayPrice = updateDisplayPrice;
        product.price = updateDisplayPrice;
      }
