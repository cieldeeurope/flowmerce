  ...
  browser = await puppeteer.launch({
    ...
    while (true) {
      try {
          ...
          const balenciagaProductSelector = 'div.l-productgrid.jsLoaded li.l-productgrid__item div.c-product__item a.c-product__focus[href]';
          let previousMetrics = await page.evaluate((selector) => ({
              height: document.body.scrollHeight,
              productCount: document.querySelectorAll(selector).length,
          }), balenciagaProductSelector);
          let stableCount = 0;

          for (let scrollAttempt = 0; scrollAttempt < 12; scrollAttempt++) {
              await this.autoScroll(page);
              await new Promise(resolve => setTimeout(resolve, 5000));
              const currentMetrics = await page.evaluate((selector) => ({
                  height: document.body.scrollHeight,
                  productCount: document.querySelectorAll(selector).length,
              }), balenciagaProductSelector);

              console.log(
                  `발렌시아가 스크롤 확인 ${scrollAttempt + 1}: height=${currentMetrics.height}, productCount=${currentMetrics.productCount}`,
              );

              if (
                  currentMetrics.height === previousMetrics.height &&
                  currentMetrics.productCount === previousMetrics.productCount
              ) {
                  stableCount++;
              } else {
                  stableCount = 0;
