        ...
        while (retryCount < 3 && !success) {
            ...
            try {
                ...
                const categories = await page.evaluate(() => {
                    ...
                    const mapLevel1: Record<string, string> = {
                        vrouw: 'WOMEN',
                        man: 'MEN'
                    };
                    ...
                    const roots = document.querySelectorAll('[data-category-name]');

                    roots.forEach(root => {

                        const raw = root.getAttribute('data-category-name')?.toLowerCase();
                        const level1 = mapLevel1[raw];

                        if (!level1) return;

                        const dropdown = root.parentElement?.querySelector('.dropdown-menu');
                        if (!dropdown) return;

                        const level2Items = dropdown.querySelectorAll(
                        '.dropdown-menu--subitem--parent'
                        );

                        level2Items.forEach(l2 => {

                        const l2Name = l2.querySelector('.subcat-menu-link .title')?.textContent?.trim();
                        if (!l2Name) return;

                        const links = l2.querySelectorAll('.container-list ul.list a[href]');
                        ...
                        links.forEach(a => {
                            ...
                            if (!name || !href) return;
                            if (/alles bekijken|ontdekken/i.test(name)) return;

                            results.push({
                            categoryName: `${level1} - ${l2Name} - ${name}`,
...
async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
    ...
    while (retryAttempts < MAX_RETRY) {
        ...
        try {
            ...
            while (true) {
                ...
                const urls = await page.evaluate(() => {
                    return Array.from(
                        ...
                        Array.from(
                            ...
                            '.product-item .product-item-image a.product-item__link[href$=".html"]'
                            )
                        )
                ...
                console.log(`page ${pageIndex} → ${urls.length}개`);

                /* =========================
                    🔥 다음 페이지 존재 체크
                ========================= */

                const hasNext = await page.evaluate((nextIndex) => {
                    return !!Array.from(
                        ...
                        return url.searchParams.get('page') === String(nextIndex);
                        } catch {
                        return false;
                        }
                    });
    ...
    console.log(`롱샴 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
        ...
        while (loadAttempts < 10) {
            try {
                ...
                console.log(`✅ (${index + 1}/${productUrls.length}) 롱샴 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                await page.waitForSelector(
                ...
                loadAttempts++;
                console.warn(`롱샴 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                if (loadAttempts < 3) {
        ...
        const productDetails = await page.evaluate(() => {
            ...
            const color = document.querySelector('.color-title .color-value')?.textContent?.trim() || '';

            const price = productJson.offers?.price ? Math.floor(parseFloat(productJson.offers.price)) : 0;
            ...
            if (select) {
                ...
                .filter(opt => opt.getAttribute('data-attr-sizeavailable') === 'false')
                .map(opt => opt.querySelector('span')?.textContent?.trim() || '')
                .filter(Boolean)
                .map(v => v.replace('DE','').trim());
            ...
            if (productJson.offers?.availability?.includes('OutOfStock')) soldOut = true;
            if (!price) soldOut = true;

            const clean = (t: string) => t?.replace(/<br\s*\/?>/gi,'\n').replace(/<[^>]+>/g,'').trim() || '';

            const desc1 = clean(productJson.description);
            const desc2 = clean(productJson.offers?.description);

            const mainInfo = [desc1, desc2].filter(Boolean).join('\n\n');

            const imageUrls = Array.from(new Set(
                Array.from(document.querySelectorAll('.product-media__inner picture img'))
                .map(img => {
            ...
            return { site, designer, title, price, color, mainInfo, styleId, brandstyleId, size, imageUrls, soldOut };

        });

        // 🔴 전부 품절
        if (!productDetails || productDetails.soldOut) {
            console.warn('롱샴 상품 품절 - 다음 productUrl로 이동');
            continue;
        }
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('롱샴 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 longchamp 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
