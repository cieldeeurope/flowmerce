    ...
    for (const siteUrl of siteUrls) {
        ...
        while (retryCount < 3 && !success) {
            ...
            try {
                ...
                if (!currentUrl.includes('en-nl')) {
                    console.log(`⚠️ 언어 리다이렉트 발생: ${currentUrl}`);
                ...
                const categories = await page.evaluate(() => {
                    ...
                    const blockedLevel1 = ['GIFTS', 'NEW ARRIVALS', 'AMI DE COEUR', "AMI'S WORLD"];
                    const blockedLevel2 = ['THE AMI SELECTION'];

                    document.querySelectorAll('script.js-gtm-json').forEach(script => {

                        try {

                        const data = JSON.parse(script.textContent || '');

                        if (!data.menu_level1 || !data.menu_level2 || !data.menu_level3) return;

                        const level1 = data.menu_level1.trim().toUpperCase();
                        const level2 = data.menu_level2.trim().toUpperCase();
                        const level3 = data.menu_level3.trim();

                        // ❌ 대분류 필터
                        if (blockedLevel1.includes(level1)) return;

                        // ❌ 중분류 필터
                        if (blockedLevel2.includes(level2)) return;

                        const link = script.closest('a.c-nav-tab__link') as HTMLAnchorElement;
                        if (!link) return;
                        ...
                        const categoryName = `${level1} - ${level2} - ${level3}`;
    ...
    while (retryAttempts < MAX_RETRY) {
        ...
        try {
            ...
            while (true) {
                ...
                const targetUrl = `${siteUrl}?page=${pageIndex}`;
                ...
                if (!currentUrl.includes(`page=${pageIndex}`)) {
                    break;
                }
                ...
                const urls: string[] = await page.evaluate(() => {
                    return Array.from(
                        document.querySelectorAll('li.u-col-span-1.text a[href]')
            ...
            productUrls = Array.from(collectedUrls);
...
console.log(`아미 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
...
for (const [index, productUrl] of productUrls.entries()) {
    ...
    while (loadAttempts < 10) {
        try {
            ...
            console.log(`✅ (${index + 1}/${productUrls.length}) 아미 ${category?.categoryName || '카테고리 없음'}  수집 중`);
            await page.waitForSelector(
                'div.c-price__regular',
                { visible: true, timeout: 30000 }
            );
            break; // ✅ 로딩 성공 시 루프 종료
        } catch (error: any) {
            loadAttempts++;
            console.warn(`아미 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
            if (loadAttempts < 3) {
        ...
        const productDetails = await page.evaluate(async () => {
            ...
            const site = 'Ami';
            const designer = '아미';
            const titleElement = document.querySelector('h1.product__title');
            const title = titleElement ? titleElement.textContent?.trim() || '' : '';
            const priceElement = document.querySelector('div.c-price__regular');
            const price = priceElement
            ? parseInt(priceElement.textContent.replace(/[^\d]/g, ''), 10)
            : null;
            const color = document.querySelector('span.u-text-black')?.textContent?.trim() || '';


            const nodes = document.querySelectorAll<HTMLElement>(
                'li[data-clrz-select-option][data-value]'
            );
            ...
            if (nodes.length) {
                const sizes = Array.from(nodes)
                    .filter(li => !(li.textContent || '').includes('Out of stock'))
            ...
            const imageUrls = Array.from(
                document.querySelectorAll<HTMLImageElement>(
                    'button.pdp__product__media img'
                )
            )
            .map(img => {
            ...
            // ✅ 1️⃣ 필요한 expandable만 선택
            const containers = document.querySelectorAll(
            'div.c-expandable.product__expandable[data-component="expandable"]'
            );
            ...
            const title = container.querySelector('button h3')?.textContent?.trim();

            // ❌ 제외
            if (!title) return;
            if (/delivery/i.test(title)) return;
            if (/responsability/i.test(title)) return;
            ...
            // ✅ 2️⃣ Made in 추출
            const madeInMatch = mainInfo.match(/made\s+in\s+([a-z\s]+?)(?:\(|\.|$)/i);
            if (madeInMatch) {
            ...
            // ✅ 3️⃣ REF → styleId 생성
            const refMatch = mainInfo.match(/REF:\s*([A-Z0-9\.]+)/i);
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('아미 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 ami 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
