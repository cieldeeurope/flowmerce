...
@Controller('cettire')
export class CettireController {
  constructor(private readonly cettireService: CettireService) {}
...
const currentCategoryCode = godoMallCategoryCode[i];
        ...
        await this.cettireService.getProductsFromCategory(currentUrl, partnerKey, apiKey, customId, accountPlatform, currentCategoryCode);
      ...
      return { message: `모든 Cettire 카테고리 수집이 성공적으로 완료되었습니다.` };
    } catch (error: any) {
      console.error(`Cettire ${currentUrl} 수집 중 오류 발생:`, error);
      throw error;
    }
  }
}
