        ...
        while (retryCount < 3 && !success) {
            ...
            try {
                ...
                const categories = await page.evaluate(() => {
                    ...
                    if (path.includes('womens')) main = 'WOMENS';
                    else if (path.includes('mens')) main = 'MENS';
                    else if (path.includes('kids')) main = 'KIDS';
                    else if (path.includes('jewelery')) main = 'JEWELERY';
                    else if (path.includes('bags')) main = 'BAGS';

                    /* -------------------------
                        1️⃣ 중분류 (product_type)
                    -------------------------- */

                    document
                        .querySelectorAll('#Details-filter\\.p\\.product_type a[data-value]')
                        .forEach(a => {
                        ...
                        const url =
                            ...
                            `?filter.p.product_type=${encodeURIComponent(name.toLowerCase())}`;
                    ...
                    document
                        .querySelectorAll('input[name="filter.p.m.filter.categories"]')
                        .forEach(input => {
                        ...
                        const childName = child.trim().toUpperCase();

                        const url =
                            `${location.origin}${location.pathname}` +
                            `?filter.p.m.filter.categories=${encodeURIComponent(value)}`;

                        if (seen.has(url)) return;
                        seen.add(url);

                        results.push({
                            categoryName: `${main} - ${parent} - ${childName}`,
                            url
                        });
    ...
    while (retryAttempts < MAX_RETRY) {
        ...
        try {
            ...
            while (true) {
                ...
                const btn = await page.$('button.js-load-more');
                ...
                try {
                    ...
                    prev => document.querySelectorAll('li.ro-grid__item').length > prev,
                    { timeout: 10000 },
                    prevCount
                    );
            ...
            productUrls = await page.evaluate(() => {
                return Array.from(
                    ...
                    Array.from(
                        ...
                        'li.ro-grid__item a[href^="/en-lux/products/"]'
                        )
                    )
                        .map(a => a.getAttribute('href'))
                        .filter(Boolean)
                        .map(href => location.origin + href.split('?')[0])
                    )
                );
            });
...
console.log(`릭오웬스 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
...
for (const [index, productUrl] of productUrls.entries()) {
    ...
    while (loadAttempts < 10) {
        try {
            ...
            console.log(`✅ (${index + 1}/${productUrls.length}) 릭오웬스 ${category?.categoryName || '카테고리 없음'}  수집 중`);
            await page.waitForSelector(
            ...
            loadAttempts++;
            console.warn(`릭오웬스 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
            if (loadAttempts < 3) {
        ...
        const productDetails = await page.evaluate(() => {
            ...
            const styleId = rawName.split('|')[0].trim();
            ...
            const mainInfo = productJson.description || '';
            ...
            const price = Math.floor(Number(available[0]?.offers?.price || 0));

            const size = available
                .map(v => {
                    const sku = v.sku || '';
                    const s = sku.split('_').pop() || '';
                    return s === 'UNI' ? '원사이즈' : s;
                })
                .join(',');
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('릭오웬스 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 rickowens 업데이트 실패: ${err.message}`);
        } finally {
            if (page && !page.isClosed()) await page.close();
            if (browser) await browser.close();
        }
