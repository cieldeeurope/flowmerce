...
// // 최종 로그 파일 경로
// const errorLogPath = path.join(errorLogDir, 'dolce_error_log.txt');
...
export class DolceService {
  ...
  private async getDolceProductUrlCount(page: Page): Promise<number> {
    return page.$$eval(
      '.GridColumn__col--_YbXQ.col.col-12 .SearchHitsItem__search-hit--Mnk4L a[href]',
      (links) =>
        new Set(
  ...
  private async clickVisibleDolceLoadMore(page: Page): Promise<boolean> {
      ...
      for (const siteUrl of urls) {
        ...
        while (retryCount < 3 && !success) {
          ...
          try {
            ...
            const dolceCategories: { categoryName: string; url: string }[] = await page.evaluate(() => {
            const firstLevelItems = document.querySelectorAll('li[data-depth="1"].has-children');
            const items: { categoryName: string; url: string }[] = [];

            firstLevelItems.forEach(firstLevelLi => {
              const mainLink = firstLevelLi.querySelector('a.navigation__link');
              if (!mainLink) return;

              const mainCategory = mainLink.getAttribute('aria-label');
              if (mainCategory !== '여성' && mainCategory !== '남성' && mainCategory !== '키즈') return;

              const megaMenu = firstLevelLi.querySelector('.mega-menu');
              if (!megaMenu) return;

              const secondLevelItems = megaMenu.querySelectorAll('li.navigation__list__item.header__navigation__list__item.has-children');

              secondLevelItems.forEach(secondLevelLi => {
                const categoryTypeSpan = secondLevelLi.querySelector('.navigation__title .navigation__text');
                const categoryType = categoryTypeSpan ? categoryTypeSpan.textContent.trim() : 'unknown';

                const subItems = secondLevelLi.querySelectorAll('li.navigation__list__item');
                subItems.forEach(subLi => {
                  const linkElement = subLi.querySelector('a.navigation__link');
                  if (!linkElement) return;

                  const link = linkElement as HTMLAnchorElement;  // 🔥 타입 캐스팅 추가
                  const subCategory = link.getAttribute('aria-label') || link.textContent.trim();
                  if (subCategory.includes('전체 보기')) return;  // 전체 보기 필터링
            ...
            allCategories.push(...dolceCategories);
            console.log(`✅ 수집 완료: ${dolceCategories.length}개`);
            success = true;
...
if (product.platform === 'smartstore' || product.platform === 'cafe24') {
  ...
  await page.evaluate(async () => {
    await new Promise<void>(resolve => {
      ...
      const interval = 300;   // 스크롤 주기
      const waitAfterBottom = 5000; // 🔥 맨 아래 도달 후 관찰 시간
      ...
      const timer = setInterval(() => {
        ...
        // ✅ 맨 아래에서 5초 동안 변화 없으면 종료
        if (Date.now() - lastChangeTime >= waitAfterBottom) {
          clearInterval(timer);
          resolve();
        }
      }, interval);
  ...
  browser = await puppeteer.launch({
    ...
    while (retryAttempts < MAX_RETRY) {
        try {
            ...
            try {
              ...
              // console.log('✅ 쿠키 거부 클릭');
              await new Promise(resolve => setTimeout(resolve, 2000));
            } catch (e: any) {
              // 쿠키 배너가 아예 안 나오는 경우
              // console.log('ℹ️ 쿠키 배너 없음 (60초 경과)');
            }
            await new Promise(resolve => setTimeout(resolve, 2000));
            await page.waitForSelector('.SearchHitsItem__search-hit--Mnk4L', {
              timeout: 30000
            });
            ...
            while (loadMoreClickCount < maxLoadMoreClicks) {
                const beforeCount = await this.getDolceProductUrlCount(page);
                const clicked = await this.clickVisibleDolceLoadMore(page);
                ...
                const afterCount = await this.getDolceProductUrlCount(page);

                if (afterCount <= beforeCount) {
                    stagnantClickCount++;

                    if (stagnantClickCount >= 2) {
                        console.log('더보기 클릭 후 상품 수 변화가 없어 반복을 종료합니다.');
                        break;
                    }
                } else {
            ...
            if (loadMoreClickCount >= maxLoadMoreClicks) {
                console.warn(`더보기 최대 클릭 수(${maxLoadMoreClicks})에 도달해 반복을 종료합니다.`);
            }
        ...
        try {
            ...
            const { currentPageProductUrls, isBlocked} = await page.evaluate(() => {
                const productUrls = Array.from(
                    new Set(
                        Array.from(document.querySelectorAll('.GridColumn__col--_YbXQ.col.col-12 .SearchHitsItem__search-hit--Mnk4L a[href]'))
    ...
    console.log(`돌체앤가바나 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
      ...
      while (loadAttempts < 10) {
        try {
            ...
            console.log(`✅ (${index + 1}/${productUrls.length}) 돌체앤가바나 ${category?.categoryName || '카테고리 없음'}  수집 중`);
      ...
      const productDetails = await page.evaluate(async (designerFromNode) => {
        ...
        const titleElement = document.querySelector('h1.ProductNameScrolling__product-name-scrolling__text-wrapper--n1o7e');
        const title = titleElement ? titleElement.textContent?.trim() || '' : '';
        const priceElement = document.querySelector<HTMLSpanElement>('span.money');
        const priceText = priceElement?.textContent?.trim().replace(/[^\d.,]/g, '') || '0';
        const price = parseFloat(priceText.replace(',', ''));
        // 돌체앤가바나는 컬러가 없음 ''로 들어가게 두기
        const color = document.querySelector('[kl-additionalpropertyname="Color"]')?.textContent?.trim() || '';
        
        const imageElements = document.querySelectorAll('.ProductGalleryItem__product-gallery-item__slide-image-wrapper--Uvga4 picture img');
        ...
        const imageUrls = Array.from(imageElements).map((img) => {
            ...
            const srcList = srcset.split(',').map((item) => {
                ...
                const url = parts[0];
                const sizeMatch = parts[1]?.match(/(\d+(\.\d+)?)x/);
                const size = sizeMatch ? parseFloat(sizeMatch[1]) : 0;
                return { url, size };
            });

            const desiredSize = 1200;
            let selectedItem = srcList.find((item) => item.size >= desiredSize / 1000); // 1x, 1.25x 단위
            if (!selectedItem && srcList.length > 0) {
                srcList.sort((a, b) => Math.abs(a.size - (desiredSize / 1000)) - Math.abs(b.size - (desiredSize / 1000)));
            ...
            // 중복 제거
            if (chosenUrl && !urlSet.has(chosenUrl)) {
                urlSet.add(chosenUrl);
                return chosenUrl;
            }
        ...
        const mainInfoContainer = document.querySelector('.ProductDetailInfoContent__pdp-info-content--ZfD12 .text');
        let mainInfo = '';
        let madeIn = '';
        let styleId = '';

        if (mainInfoContainer) {
            const paragraphs = mainInfoContainer.querySelectorAll('p');
            const resultLines: string[] = [];

            paragraphs.forEach(p => {
                const splitLines = p.innerHTML.split('<br>').map(line => line.replace(/<[^>]*>/g, '').trim());
                resultLines.push(...splitLines);
            });

            mainInfo = resultLines.filter(line => line.length > 0).join('\n');

            // 1️⃣ 한글 버전: • 제조국: 이탈리아
            const madeInKrMatch = mainInfo.match(/•\s*제조국:\s*([^\n]+)/);
            if (madeInKrMatch) {
              madeIn = madeInKrMatch[1].trim();

            } else {
              // 2️⃣ 영문 버전: Made in Italy
              const madeInEnMatch = mainInfo.match(/Made in\s+([^\n]+)/i);
              if (madeInEnMatch) {
                madeIn = madeInEnMatch[1].trim();

              } else {
                // 3️⃣ 한글 변형: "이탈리아 제조", "프랑스 제조" 등
                const madeInKrLooseMatch = mainInfo.match(/([^\n•]+?)\s*제조/);
                if (madeInKrLooseMatch) {
                  madeIn = madeInKrLooseMatch[1].trim();
                }
              }
            }
        }

        const styleIdElement = document.querySelector('.pdp-info__sku-message');
        if (styleIdElement) {
            const styleIdMatch = styleIdElement.textContent?.match(/Art\. Nr\.\s*([A-Za-z0-9]+)/);
            if (styleIdMatch) {
        ...
        // ⚡ 사이즈 선택 버튼 클릭 및 사이즈 추출
        let size: string | null = null;
        const sizeButton = document.querySelector('.ProductStrip__product__strip__size__cta--PeXIV') as HTMLElement;
        if (sizeButton) {
            sizeButton.click();
            console.log('⚡ 사이즈 선택 버튼 클릭됨');
            await new Promise(resolve => setTimeout(resolve, 3000)); // 3초 대기 (Flyout 로딩)
        }

        const sizeListElement = document.querySelector('.FlyoutSizeSelectorValues__flyout-size-selector__list--ttG1j');
        if (!sizeListElement) {
            size = '원사이즈';
        } else {
            const sizeItems = sizeListElement.querySelectorAll('li.FlyoutSizeSelectorValues__flyout-size-selector__list-item--eWDKH');
            const availableSizes = Array.from(sizeItems)
                .filter(li => !li.classList.contains('FlyoutSizeSelectorValues__not-orderable--uJxkK'))
      ...
      if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
        console.warn('돌체앤가바나 데이터 누락 - 다음 productUrl로 이동');
        ...
        // ) {
        //   console.warn('❌ 돌체앤가바나 데이터 누락 상세');

        //   if (!productDetails) {
        //     console.warn(' - productDetails 자체가 null/undefined');
        //   } else {
        //     console.warn(' - title:', productDetails.title);
        //     console.warn('   → title falsy?', !productDetails.title);

        //     console.warn(' - price:', productDetails.price);
        //     console.warn('   → price falsy?', !productDetails.price);

        //     console.warn(' - mainInfo:', productDetails.mainInfo);
        //     console.warn('   → mainInfo falsy?', !productDetails.mainInfo);

        //     console.warn(' - styleId:', productDetails.styleId);
        //     console.warn('   → styleId falsy?', !productDetails.styleId);

        //     console.warn(' - brandstyleId:', productDetails.brandstyleId);
        //     console.warn('   → brandstyleId falsy?', !productDetails.brandstyleId);

        //     console.warn(' - size:', productDetails.size);
        //     console.warn('   → size falsy?', !productDetails.size);

        //     console.warn(' - imageUrls.length:', productDetails.imageUrls?.length);
        //     console.warn(
        //       '   → imageUrls.length falsy?',
        //       !productDetails.imageUrls || !productDetails.imageUrls.length
        //     );
        //   }

        //   console.warn('❌ 다음 productUrl로 이동');
        //   continue;
        // }
...
const unsoldProducts = await this.productRepository.find({
          ...
          for (const product of unsoldProducts) {
              ...
              console.log(`돌체앤가바나 품절 처리 완료: ${product.title} (styleId: ${product.styleId})`);
          }
          }
...
const apiKey = account.apiKey;
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
        ...
        console.warn(`🚨 Dolce 업데이트 실패: ${err.message}`);
