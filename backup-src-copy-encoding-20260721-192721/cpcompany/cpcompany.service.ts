                ...
                try {
                    ...
                    const categories = await page.evaluate((): { categoryName: string; url: string; needsTranslation?: boolean }[] => {
                        ...
                        if (host.includes('cpcompany.com')) {
                            ...
                            document.querySelectorAll('a[href*="/shop/metropolis-collection/"]').forEach(element => {
                                const anchor = element as HTMLAnchorElement;
                                const href = anchor.getAttribute('href');
                                if (!href) return;
                                ...
                                const url = new URL(href, location.origin);
                                const segments = url.pathname.toLowerCase().split('/').filter(Boolean);
                                const baseIndex = segments.indexOf('metropolis-collection');
                                const slug = segments[baseIndex + 1];
                                if (!slug || excludedSlugs.has(slug)) return;

                                const name = normalize(anchor.textContent) || titleCaseSlug(slug);
                                if (isBadName(name) || isAllCategoryName(name)) return;

                                addCategory(`METROPOLIS COLLECTION - ${name}`, href);
                            });
                        }
    ...
    async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
        ...
        while (retryAttempts < MAX_RETRY) {
            ...
            try {
                ...
                const collectedProductUrls = new Set<string>();
                ...
                for (let pageIndex = 0; pageIndex < 50; pageIndex++) {
                    ...
                    await page.waitForSelector('.product-tile-container.cp-product-tile .product[data-pid], .product[data-pid] a[href$=".html"], .pdp-link a[href]', {
                        timeout: 30000
                    });
                    ...
                    const pageResult = await page.evaluate(() => {
                        ...
                        document.querySelectorAll('.product-tile-container.cp-product-tile, .product[data-pid]').forEach(el => {
                            const tile = el.querySelector('.product-tile');
                            if (tile?.classList.contains('sold-out')) return;

                            el.querySelectorAll('.pdp-link a[href], a[href$=".html"], a[href*=".html?"]').forEach(anchor => {
                                addUrl(anchor.getAttribute('href'));
                            });
                        });

                        const nextAnchor = Array.from(document.querySelectorAll('a[href*="start="]')).find(anchor => {
                            const button = anchor.querySelector('button');
                            const html = anchor.innerHTML || '';
                            return !button?.hasAttribute('disabled') && html.includes('ArrowRight');
                        }) as HTMLAnchorElement | undefined;

                        return {
                            urls: Array.from(urls),
                            nextPageUrl: nextAnchor?.href || null,
                        };
                    });

                    pageResult.urls.forEach(url => collectedProductUrls.add(url));
                    if (!pageResult.nextPageUrl || pageResult.nextPageUrl === currentPageUrl) break;
                    currentPageUrl = pageResult.nextPageUrl;
                }

                productUrls = Array.from(collectedProductUrls);
                break;
            }   catch (error: any) {
        ...
        console.log(`CP컴퍼니 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
        ...
        for (const [index, productUrl] of productUrls.entries()) {
            ...
            while (loadAttempts < 10) {
                try {
                    ...
                    console.log(`✅ (${index + 1}/${productUrls.length}) CP컴퍼니 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                    await page.waitForSelector(
                        'script[type="application/ld+json"], [data-attr="size"], .custom-color-pdp',
                        { timeout: 10000 }
                    ).catch(() => {});
                    break; // ✅ 로딩 성공 시 루프 종료
                } catch (error: any) {
                    loadAttempts++;
                    console.warn(`CP컴퍼니 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                    if (loadAttempts < 3) {
            ...
            const productDetails = await page.evaluate(() => {
                ...
                const site = 'CPcompany';
                const designer = 'CP컴퍼니';
                const oneSize = '원사이즈';
                ...
                const currentSku = location.pathname.match(/\/([A-Z0-9]+)\.html/i)?.[1] || '';
                ...
                const productJson =
                    jsonItems.find(item => {
                        const typeValue = Array.isArray(item?.['@type'])
                            ? item['@type'].join(' ')
                            : item?.['@type'];
                        const isProduct = normalizeText(String(typeValue || '')).toLowerCase().includes('product');
                        const ids = [item?.sku, item?.mpn].filter(Boolean).map(String);
                        return isProduct && (!currentSku || ids.includes(currentSku));
                    }) ||
                    jsonItems.find(item =>
                        normalizeText(String(item?.['@type'] || '')).toLowerCase().includes('product')
                    ) ||
                    {};

                const styleId = normalizeText(String(productJson.sku || productJson.mpn || currentSku || ''));
                const brandstyleId = styleId;
                const title =
                    normalizeText(String(productJson.name || '')) ||
                    normalizeText(document.querySelector('h1.product-name, h1')?.textContent);
                const price = parsePrice(productJson?.offers?.price);
                const color = normalizeText(document.querySelector('.custom-color-pdp')?.textContent);
                const mainInfo = normalizeText(String(productJson.description || ''));
                const madeInMatch = mainInfo.match(/made\s+in\s+([A-Za-z]+)/i);
                const madeIn = madeInMatch?.[1] || '';
                const availability = normalizeText(String(productJson?.offers?.availability || '')).toLowerCase();
                let soldOut =
                ...
                const rawImages = Array.isArray(productJson.image) ? productJson.image : [productJson.image];
                ...
                const sizeContainers = Array.from(
                    document.querySelectorAll('[data-attr="size"] .swatch-attribute-container')
                );

                const sizes = Array.from(new Set(
                    sizeContainers
                        .filter(el => {
                            const classes = Array.from(el.classList).map(className => className.toLowerCase());
                            if (classes.includes('is-restockable')) return false;
                            if (classes.includes('unselectable') || classes.includes('disabled')) return false;
                            if (el.getAttribute('aria-disabled') === 'true') return false;
                            return true;
                        })
                        .map(el => {
                            const value =
                                el.querySelector('[data-display-value]')?.getAttribute('data-display-value') ||
                                el.querySelector('[data-attr-value]')?.getAttribute('data-attr-value') ||
                                el.querySelector('.attr-value')?.textContent ||
                                el.getAttribute('aria-describedby') ||
                                '';
                            const clean = normalizeText(value);
                            return clean.toUpperCase() === 'ONESIZE' ? oneSize : clean;
                        })
                        .filter(Boolean)
                ));
            ...
            if (!productDetails || productDetails.soldOut) {
                console.warn('CP컴퍼니 상품 품절 - 다음 productUrl로 이동');
                continue;
            }
            ...
            if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
                console.warn('CP컴퍼니 데이터 누락 - 다음 productUrl로 이동');
    ...
    async getProductUpdate(visitUrl: string, goodsNo: string, customId: string, accountPlatform: string){
        ...
        for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
            ...
            try {
                ...
                console.warn(`🚨 cpcompany 업데이트 실패: ${err.message}`);
            } finally {
                if (page && !page.isClosed()) await page.close();
                if (browser) await browser.close();
            }
