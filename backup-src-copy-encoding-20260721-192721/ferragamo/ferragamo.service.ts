...
// // 최종 로그 파일 경로
// const errorLogPath = path.join(errorLogDir, 'ferragamo_error_log.txt');
      ...
      for (const siteUrl of urls) {
        ...
        while (retryCount < 3 && !success) {
          ...
          try {
            ...
            const ferragamoCategories: { categoryName: string; url: string }[] = await page.evaluate(() => {
            const firstLevelItems = document.querySelectorAll('ul.main-menu__tree-list > li.main-menu__tree-first-level');
            const items: { categoryName: string; url: string }[] = [];

            firstLevelItems.forEach(firstLevelLi => {
              const mainLink = firstLevelLi.querySelector('a.main-menu__tree-link');
              if (!mainLink) return;

              const mainCategory = mainLink.textContent.trim();
              if (mainCategory !== 'Women' && mainCategory !== 'Men') return;

              const categorySections = firstLevelLi.querySelectorAll('li[data-bg-color]');

              categorySections.forEach(sectionLi => {
                const categoryTypeSpan = sectionLi.querySelector('a.main-menu__tree-link--title span');
                const categoryType = categoryTypeSpan ? categoryTypeSpan.textContent.trim().toLowerCase() : 'unknown';

                const subList = sectionLi.querySelector('ul.main-menu__tree-list--ul');
                if (!subList) return;

                const subItems = subList.querySelectorAll('li.main-menu__tree-link--item a.main-menu__tree-link');
                subItems.forEach(subLinkElement => {
                  const subLink = subLinkElement as HTMLAnchorElement;  // 타입 캐스팅
                  const subCategory = subLink.textContent.trim().toLowerCase();
                  if (subCategory.includes('highlights') || subCategory.includes('view all') || subCategory.includes('viewall')) return;

                  const categoryName = `${mainCategory} - ${categoryType} - ${subCategory.replace(/ /g, '')}`;
                  const url = subLink.href;

                  items.push({ categoryName, url });
                });
              });
            });
          ...
          allCategories.push(...ferragamoCategories);
          console.log(`✅ 수집 완료: ${ferragamoCategories.length}개`);
          success = true;
  ...
  browser = await puppeteer.launch({
    ...
    while (retryAttempts < MAX_RETRY) {
        try {
            ...
            const seeMoreButton = await page.$('button.see-more.btn.cmp-button.cmp-button--fill');
            if (seeMoreButton) {
                const hasHiddenClass = await page.evaluate(el => el.classList.contains('hidden'), seeMoreButton);
    ...
    while (true) {
        try {
            ...
            const { currentPageProductUrls, isBlocked } = await page.evaluate(() => {
              // 1차: 정확히 class가 'r23-grid--list-plp__item' 만 있는 li 요소 필터링
              const exactMatchElements = Array.from(
                document.querySelectorAll('li.r23-grid--list-plp__item')
              ).filter(
                el => el.classList.length === 1 && el.classList.contains('r23-grid--list-plp__item')
              );

              let productUrls = exactMatchElements
                .map(el => {
              ...
              // 👉 만약 수집된게 없다면 fallback 실행
              if (productUrls.length === 0) {
                const fallbackLinks = document.querySelectorAll('.product-list-r23__item__inner a[href]');
                productUrls = Array.from(fallbackLinks)
    ...
    console.log(`페라가모 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
      ...
      while (loadAttempts < 10) {
        try {
            ...
            console.log(`✅ (${index + 1}/${productUrls.length}) 페라가모 ${category?.categoryName || '카테고리 없음'}  수집 중`);
            await new Promise((resolve) => setTimeout(resolve, 2000));
            success = true;
            break; // 로딩 성공 시 루프 종료
          } catch (error: any) {
      ...
      const productDetails = await page.evaluate(async () => {
        const site = 'Ferragamo';
        const designer = '페라가모';
        const titleElement = document.querySelector('h1.prod-r23-name');
        const title = titleElement ? titleElement.textContent?.trim() || '' : '';
        const priceElement = document.querySelector('li.prod-r23-prices__item');
        const price = priceElement ? parseFloat(priceElement.getAttribute('data-item-price') || '0') : 0;
        const color = document.querySelector('span.product-toolbar__attribute-color-label.sr-only')?.textContent?.trim() || '';
        
        // 1. 대표 이미지 (현재 활성화된 슬라이드)
        const activeAnchor = document.querySelector('.product-gallery-r23__items__container.slick-current.slick-active a[href]');
        const mainImage = activeAnchor ? activeAnchor.getAttribute('href') : null;

        // 2. 모든 이미지 URL 추출 (중복제거)
        const allAnchors = document.querySelectorAll('.product-gallery-r23__items__container a[href]');
        const imageUrls = Array.from(allAnchors)
          .map(anchor => anchor.getAttribute('href'))
          .filter((href, index, self) => href && self.indexOf(href) === index);

        // 3. 대표이미지를 맨 앞으로 정렬
        if (mainImage) {
          const mainIndex = imageUrls.indexOf(mainImage);
          if (mainIndex > 0) {
            imageUrls.splice(mainIndex, 1); // 중복 제거
            imageUrls.unshift(mainImage);   // 맨 앞에 삽입
          }
        }


        const sizeElements = document.querySelectorAll('#ae_sizeCombo li');
        let size;
        if (!sizeElements || sizeElements.length === 0) {
            ...
            const sizeList = Array.from(sizeElements)
                .map(li => {
                    const input = li.querySelector('input[data-attr-size]');
                    return input ? input.getAttribute('data-attr-size') : '';
                })
                .filter(value => value.trim().length > 0);

            if (sizeList.length === 0) {
                console.log("⚠️ 사이즈가 없습니다.");
                size = null;
            } else {
                size = sizeList.join(', ');
            }
        ...
        // Long Description
        const longDescriptionElement = document.querySelector('div[data-id="longDescription"] p') as HTMLElement;
        if (longDescriptionElement) {
          mainInfo += longDescriptionElement.innerText.trim() + '\n\n';
        }

        // Short Description 리스트 추가
        const listItems = document.querySelectorAll('div[data-id="shortDescription"] li') as NodeListOf<HTMLElement>;
        listItems.forEach((li) => {
          mainInfo += li.innerText.trim() + '\n';
        });
        mainInfo = mainInfo.trim();

        // styleId 추출 (Product code li 직접 찾기)
        const styleIdElement = document.querySelector('li.prod-r23-model-code') as HTMLElement;
        const styleId = styleIdElement ? styleIdElement.innerText.replace('Product code:', '').trim() : '';
        const brandstyleId = styleId;

        // madeIn 추출 (Made In 으로 시작하는 li 찾기)
        const madeInLi = Array.from(listItems).find(li => li.innerText.trim().startsWith('Made In'));
        const madeIn = madeInLi ? madeInLi.innerText.replace('Made In', '').trim() : '';
      ...
      if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
        console.warn('페라가모 데이터 누락 - 다음 productUrl로 이동');
...
const unsoldProducts = await this.productRepository.find({
          ...
          for (const product of unsoldProducts) {
              ...
              console.log(`페라가모 품절 처리 완료: ${product.title} (styleId: ${product.styleId})`);
          }
          }
...
const apiKey = account.apiKey;
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
        ...
        console.warn(`🚨 Ferragamo 업데이트 실패: ${err.message}`);
