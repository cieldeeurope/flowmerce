            ...
            while (retryCount < 3 && !success) {
                ...
                try {
                    ...
                    const categories = await page.evaluate((): { categoryName: string; url: string }[] => {
                        ...
                        const topFromSectionId = (id: string): string => {
                            if (id.startsWith('WOMAN')) return 'WOMEN';
                            if (id.startsWith('MAN')) return 'MEN';
                            if (id.startsWith('KID')) return 'KIDS';
                            return '';
                        };

                        document.querySelectorAll('h2[id]').forEach(element => {
                            const heading = element as HTMLElement;
                            const sectionId = (heading.getAttribute('id') || '').toUpperCase();
                            if (!sectionId || sectionId.startsWith('SALE')) return;

                            const top = topFromSectionId(sectionId);
                            if (!top) return;

                            const middleName = cleanName(heading.textContent);
                            if (isSkipName(middleName)) return;

                            let list = heading.nextElementSibling;
                            while (list && !list.classList.contains('category-section-list')) {
                                list = list.nextElementSibling;
                            }
                            if (!list) return;

                            list.querySelectorAll('a.category-section-list-item[href]').forEach(link => {
                                const anchor = link as HTMLAnchorElement;
                                const href = anchor.getAttribute('href');
                                const name = cleanName(anchor.textContent);
                                if (!href || isSkipName(name)) return;
                                addCategory(top + ' - ' + middleName + ' - ' + name, href);
                            });
                        });
    ...
    async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
        ...
        while (retryAttempts < MAX_RETRY) {
            ...
            try {
                ...
                const productSelector = 'article.producttile, .producttile-cover[href], .producttile-name a[href]';
                const extractProductUrlsFromHtml = (html: string) => {
                    ...
                    $('article.producttile').each((_, element) => {
                        const tile = $(element);
                        const tileText = normalizeText(tile.text()).toLowerCase();
                        const notifyCount = tile.find('.modal-sizes-cta-info').filter((__, info) =>
                            normalizeText($(info).text()).toLowerCase().includes('notify me')
                        ).length;
                        const hasNotify =
                            notifyCount > 0 ||
                            tileText.includes('notify me') ||
                            tile.find('.btn-addtocart--notifyme').length > 0;
                        const hasAvailableSize =
                            tile.find('[data-option-available="true"], .modal-sizes-cta:not(.not-available):not([disabled])').length > 0;
                        if (hasNotify && !hasAvailableSize) return;

                        tile.find('a.producttile-cover[href], .producttile-name a[href], a.producttile-gallery-inner[href]').each((__, anchor) => {
                            addUrl($(anchor).attr('href'));
                        });
                    });

                    if (!urls.size) {
                        $('a.producttile-cover[href], .producttile-name a[href], a.producttile-gallery-inner[href]').each((_, anchor) => {
                            addUrl($(anchor).attr('href'));
                        });
                    }
                ...
                const listingResponse = await page.goto(listingUrl, { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(() => null);
                const responseHtml = await listingResponse?.text().catch(() => '') || '';
                productUrls = extractProductUrlsFromHtml(responseHtml);

                if (!productUrls.length) {
                    await page.waitForSelector(productSelector, { timeout: 30000 });
                    const renderedHtml = await page.content();
                    productUrls = extractProductUrlsFromHtml(renderedHtml);
                }
                break;
            }   catch (error: any) {
        ...
        console.log(`에트로 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
        ...
        for (let index = 0; index < productUrls.length; index++) {
            ...
            const retryCurrentProduct = async (reason: string) => {
                ...
                console.warn(`에트로 상품 상세 재시도 (${retryCount}/3): ${productUrl} / ${reason}`);
                try {
                ...
                console.warn(`에트로 상품 상세 재시도 초과 - 다음 productUrl로 이동: ${productUrl}`);
                return false;
            };
            let loadAttempts = 0;
            ...
            while (loadAttempts < 10) {
                try {
                    ...
                    console.log(`✅ (${index + 1}/${productUrls.length}) 에트로 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                    ...
                    loadAttempts++;
                    console.warn(`에트로 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                    if (loadAttempts < 3) {
                        if (page && !page.isClosed()) await page.close();
                        if (browser) await browser.close();
            ...
            const productDetails = await page.evaluate(async () => {
                ...
                const productJson = productGroups.find(matchesPageSku) || productGroups[0] || productObjects.find(matchesPageSku) || productObjects[0] || {};
            ...
            const extractSize = () => {
                ...
                const options = Array.from(document.querySelectorAll('[role="listbox"][id^="listbox-selectsize"] li[role="option"], ul[id^="listbox-selectsize"] li[role="option"]'));
                if (!options.length) return { size: oneSize, soldOut: false };
                const isInvalidSize = (value: string) => {
                    const lower = normalizeText(value).toLowerCase();
                    return !lower ||
                        lower === 'select size' ||
                        lower === 'select a size' ||
                        lower === 'size' ||
                        /^(white|light blue|fuchsia|black|blue|green|pink|red|yellow|brown|beige|grey|gray|navy|orange|purple|ivory|cream|gold|silver|multicolor)$/i.test(lower);
                };
                const sizes = options
                    .filter(option => option.closest('[id^="listbox-selectsize"]'))
                    ...
                    .filter(size => Boolean(size) && !isInvalidSize(size));
            ...
            const brandstyleId = styleId;
                ...
                const color = normalizeText(inStockVariants.find((variant: any) => normalizeText(variant.color))?.color || variants.find((variant: any) => normalizeText(variant.color))?.color || productJson.color);
                
            const filteredImages = Array.from(new Set(extractSchemaImages(productJson)))
            ...
            pushInfo(infoParts, productJson.material);
                ...
                const madeIn = extractMadeIn(mainInfo);
            ...
            if (!productDetails || productDetails.soldOut) {
                console.warn('에트로 상품 품절 - 다음 productUrl로 이동');
                continue;
            }
            ...
            if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
                console.warn('에트로 데이터 누락 - 다음 productUrl로 이동:', productUrl);
                console.warn('에트로 누락 상세:', {
    ...
    async getProductUpdate(visitUrl: string, goodsNo: string, customId: string, accountPlatform: string){
        ...
        for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
            ...
            try {
                ...
                console.warn(`🚨 etro 업데이트 실패: ${err.message}`);
            } finally {
                if (page && !page.isClosed()) await page.close();
                if (browser) await browser.close();
            }
