...
// // 최종 로그 파일 경로
// const errorLogPath = path.join(errorLogDir, 'stone_error_log.txt');
    ...
    for (const siteUrl of urls) {
      ...
      while (retryCount < 3 && !success) {
        ...
        try {
          ...
          // 🍪 쿠키 처리
          try {
            await page.waitForSelector('#onetrust-banner-sdk', { timeout: 3000 });

            const rejectBtn = await page.$('.onetrust-close-btn-handler');
          ...
          // 🔥 collections 클릭 (먼저 해야됨)
          let clicked = false;

          try {
            await page.waitForSelector('button.menu-heading', { timeout: 5000 });

            const buttons = await page.$$('button.menu-heading');

            for (const btn of buttons) {
              const text = await page.evaluate(el => el.textContent?.trim(), btn);

              if (text?.toLowerCase() === 'collections') {
                console.log('📂 collections 클릭');
                await btn.click();
                clicked = true;
                break;
              }
            ...
            console.log('⚠️ collections 버튼 못찾음');
          }

          // 🔥 메뉴 뜰 때까지 기다림 (클릭 후!)
          try {
            await page.waitForSelector('li.menu-voice', { timeout: 10000 });
          } catch {
            console.log('⚠️ menu-voice 안뜸');
          }

          // 🔥 카테고리 추출
          const categories = await page.evaluate(() => {

            const result: { categoryName: string; url: string }[] = [];

            // 🔥 여러 메뉴 대비
            const menus = document.querySelectorAll('li.menu-voice');

            menus.forEach((root) => {
              const anchors = root.querySelectorAll(
                'ul.submenu-voice__list li.submenu-voice a'
              );
              ...
              anchors.forEach((a) => {
                ...
                if (name.toUpperCase().includes('VIEW ALL')) return;
          ...
          console.log(`✅ ${categories.length}개 수집`);
          allCategories.push(...categories);
          success = true;

        } catch (err) {
          console.error('❌ 메인 에러:', err);
          retryCount++;
        } finally {
          try {
            await browser.close();
          } catch {
            console.log('⚠️ browser.close 실패 무시');
          }
        }
      }
    }
  ...
  browser = await puppeteer.launch({
    ...
    while (retryAttempts < MAX_RETRY) {
        try {
            ...
            // ✅ siteUrl 기준으로 designer 결정
            designerFromNode = isKids
              ? '스톤아일랜드 키즈'
              : '스톤아일랜드';
            await new Promise(resolve => setTimeout(resolve, 3000));
            await page.evaluate(() => {
                const moreButton = document.querySelector('button.button.button--secondary.full');
        ...
        try {
            ...
            const { currentPageProductUrls, isBlocked } = await page.evaluate(() => {
              const productUrls = Array.from(
                  document.querySelectorAll('div.product-tile__info_content')
              )
              .filter(productTile => {
                  // Notify Me 버튼 존재 여부 확인
                  const notifyButtonText = productTile.querySelector('div.product-tile__quite button span')?.textContent?.trim() || '';
                  return notifyButtonText !== 'Notify Me'; // ✅ "Notify Me" 가 아닌 것만 통과
              })
              .map(productTile => {
                  const linkEl = productTile.querySelector('div.product-tile__info_base a[href]') as HTMLAnchorElement;
                  return linkEl?.href || '';
              })
    ...
    console.log(`✅ 스톤아일랜드 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
      ...
      while (loadAttempts < 10) {
        try {
            ...
            console.log(`✅ (${index + 1}/${productUrls.length}) 스톤아일랜드 ${category?.categoryName || '카테고리 없음'}  수집 중`);
            await new Promise((resolve) => setTimeout(resolve, 2000));
            try {
                // "Continue without Accepting" 버튼 기다리기
                await page.waitForSelector('.onetrust-close-btn-handler', { visible: true, timeout: 6000 });
                const continueButton = await page.$('.onetrust-close-btn-handler');
                if (continueButton) {
                    console.log('쿠키 버튼 클릭');
                    await continueButton.click();
                    await new Promise((resolve) => setTimeout(resolve, 2000));
                }
            } catch (error: any) {
                console.log('쿠키 버튼 생략');
            }
      ...
      const hasNotify = await page.evaluate(() => {
        // "Notify Me" 버튼 여부 확인
        const btns = Array.from(document.querySelectorAll('button.button.button--primary span'));
        return btns.some(span => span.textContent?.trim() === "Notify Me");
      });

      if (hasNotify) {
        continue; // 재고없음이면 다음 상품으로 건너뜀
      }
      ...
      const productDetails = await page.evaluate(async (designerFromNode) => {
        ...
        const titleElement = document.querySelector('h1.product-selection__name');
        const title = titleElement ? titleElement.textContent?.trim() || '' : '';
        
        let price = 0;
        const priceWrap = document.querySelector('span.product-price-wrap');

        if (priceWrap) {

          const hasOriginalPrice = priceWrap.querySelector('del.product-price-list');

          if (hasOriginalPrice) {

            const saleEl = priceWrap.querySelector('span.product-price-sale');
        ...
        /* 🔥 fallback : 키즈 "From €380" 구조 */

        if (!price || isNaN(price)) {

          const altPriceEl = document.querySelector(
            'span.product-selection__price span.product-price'
          );

          if (altPriceEl) {

            price = parseFloat(
              altPriceEl.textContent
                ?.replace(/[^\d,]/g, '')
                .replace(/\./g, '')
                .replace(',', '.')
            );
        ...
        const color = document.querySelector('span.selector__label__value')?.textContent?.trim() || '';
        
        const imageElements = document.querySelectorAll('ul.row.product-hero-gallery li.col-12 img[data-quality-img]');

        const imageUrls = Array.from(imageElements)
        .map((img) => {
            const rawUrl = img.getAttribute('data-quality-img') || '';
            return rawUrl.split('?')[0]; // ✅ 쿼리스트링 제거
        })
        .filter((url) => url !== '');

        const sizeButton = document.querySelector(
          '.selector-size-dropdown__button'
        ) as HTMLElement | null;
        ...
        // 🔥 1️⃣ disabled 상태면 무조건 품절
        if (sizeButton.hasAttribute('disabled')) {
          console.log('❌ 사이즈 버튼 disabled → 품절');
          return null;
        }

        // 🔥 2️⃣ 텍스트가 Out of stock 인 경우도 품절
        const btnText = sizeButton.textContent?.trim().toLowerCase() || '';
        if (btnText.includes('out of stock')) {
          console.log('❌ Out of stock 버튼 감지 → 품절');
          return null;
        }
        ...
        const listItems = document.querySelectorAll(
          '#size-selector-container li.size-selector-line'
        );
        ...
        const available = Array.from(listItems).filter(
        ...
        // Step 1. "More Details" 버튼 클릭
    const moreDetailsButton = document.querySelector('button.button.button--tertiary.product-details__button');
    if (moreDetailsButton) {
        (moreDetailsButton as HTMLElement).click();
        await new Promise(resolve => setTimeout(resolve, 1000));
    }

    // Step 2. accordion__item 요소들 수집
    const accordionItems = Array.from(document.querySelectorAll('div.product-details div.accordion__item'));

    for (const item of accordionItems) {
        const headerButton = item.querySelector('button.accordion__header');
        if (!headerButton) continue;

        const id = headerButton.id;
        if (id === 'shipping-returns') continue;
        ...
        const panel = item.querySelector('div.accordion__panel') as HTMLElement;
        if (panel) {
            // Agec 요소 제거
            const agecElement = panel.querySelector('div.composition-care_agec');
            if (agecElement) {
                agecElement.remove();
            }
        ...
        // Step 4. Made in 추출 (대소문자 정확히 일치)
        const madeInMatch = mainInfo.match(/\bMade in\s+([^\n]+)/);
        if (madeInMatch) {
      ...
      if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
        console.warn('스톤아일랜드 데이터 누락 - 다음 productUrl로 이동');
...
const unsoldProducts = await this.productRepository.find({
          ...
          for (const product of unsoldProducts) {
              ...
              console.log(`스톤아일랜드 품절 처리 완료: ${product.title} (styleId: ${product.styleId})`);
          }
          }
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        if (currentUrl.includes('/new-arrivals')) {
          ...
          await page.waitForSelector(
            'span.product-price-wrap',
            { visible: true, timeout: 30000 }
          );
          
          productDetails = await page.evaluate(async () => {
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
        ...
        console.warn(`🚨 Stone 업데이트 실패: ${err.message}`);
