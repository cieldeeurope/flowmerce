...
import plainPuppeteer from 'puppeteer';
    ...
    for (const siteUrl of siteUrls) {
        ...
        while (retryCount < 3 && !success) {
            ...
            page.on('request', (request) => {
                try {
                if (
                    typeof (request as any).isInterceptResolutionHandled === 'function' &&
            ...
            try {
                ...
                const categories = await page.evaluate(() => {
                    const targetIds = [
                        'l1_navigation_item_2',
                        'l1_navigation_item_3'
                    ];
                    ...
                    targetIds.forEach(id => {
                        const l1 = document.getElementById(id);
                        if (!l1) return;

                        const l1Name =
                        l1.querySelector('span')?.textContent?.trim() || '';

                        const container = l1.closest('li')?.querySelector('[data-testid^="menu-container"]');
                        if (!container) return;

                        const level2List = container.querySelectorAll('li[data-menulevel="l2"]');

                        level2List.forEach(l2 => {

                        const l2Name =
                            l2.querySelector('._label_16kbx_41')?.textContent?.trim() ||
                            l2.querySelector('a span')?.textContent?.trim() ||
                            '';

                        if (excludeKeywords.some(k => l2Name.toLowerCase().includes(k))) return;

                        const links = l2.querySelectorAll('li[data-menulevel="l3"] a[href]');
                        ...
                        links.forEach(a => {
                            ...
                            if (excludeKeywords.some(k => lowerName.includes(k))) return;

                            results.push({
                            categoryName: `${l1Name} - ${l2Name} - ${name}`,
    ...
    for (let i = 1; i <= 3; i++) {
      try {
        const response = await axios.get(imageUrl, {
          ...
          timeout: 60000, // 🔥 30 → 60으로 증가
          headers: {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
            "Accept": "image/avif,image/webp,image/apng,image/*,*/*;q=0.8",
            "Referer": "https://www.gucci.com/", // 🔥 핵심
          }
        });
        ...
        console.warn(`⚠️ Gucci 이미지 다운로드 retry (${i}) → ${imageUrl}`);
...
async getProductsFromCategory(siteUrl: string, partnerKey: string,apiKey: string, customId:string, accountPlatform:string, godoMallCategoryCode: string): Promise<void> {
    ...
    while (retryAttempts < MAX_RETRY) {
        ...
        try {
            ...
            await page.waitForSelector(
                'a.ProductGrid_product_card_link__vuwW_',
                { timeout: 30000 }
            );
            while (true) {
                try {
                    const moreBtn = await page.$('a[data-testid="load-all"]');
                    ...
                    // 👉 클릭 (가끔 puppeteer click 안 먹음 → evaluate 추천)
            ...
            productUrls = await page.evaluate(() => {
                return Array.from(
                    ...
                    Array.from(
                        document.querySelectorAll(
                        'a.ProductGrid_product_card_link__vuwW_[href]'
                        )
                    )
    ...
    console.log(`구찌 최종 수집된 상품 URL 수: ${productUrls.length} - ${category?.categoryName || '카테고리 없음'}`);
    ...
    for (const [index, productUrl] of productUrls.entries()) {
        ...
        while (loadAttempts < 10) {
            try {
                ...
                console.log(`✅ (${index + 1}/${productUrls.length}) 구찌 ${category?.categoryName || '카테고리 없음'}  수집 중`);
                await page.waitForSelector(
                ...
                loadAttempts++;
                console.warn(`구찌 페이지 로드 실패 [${error.message}] (프록시 변경 ${loadAttempts}/3): ${error.message}`);
                if (loadAttempts < 3) {
        ...
        const productDetails = await page.evaluate(() => {
            ...
            const discoverBtn = document.querySelector(
                '[data-testid="discover-similar-products-cta"]'
            );
            ...
            const title = productJson.name || '';

            const styleId = productJson.sku || '';
            const brandstyleId = styleId;

            let imageUrls = (productJson.image || []).filter(Boolean);
            ...
            const color =
                ...
                '.variations-container_variation-selector__current-variation-value__wYvdr'
                )?.textContent?.trim() || '';
            ...
            const validOffers = offerList.filter(o =>
                ...
                o?.availability?.includes('OnlineOnly')
            );
            ...
            if (validOffers.length) {
                ...
                ? Math.floor(parseFloat(validOffers[0].price))
                : 0;
            }
            ...
            if (offerList.length > 1) {
                ...
                if (sku.includes('_')) {
                    const extracted = sku.split('_')[1]?.trim();
                    if (extracted) sizeSet.add(extracted);
                }
                });
            ...
            if (!validOffers.length) soldOut = true;
            if (!price) soldOut = true;

            /* =========================
            8️⃣ JSON description
            ========================= */

            const jsonDesc = productJson.description
            ...
            const items = Array.from(
                document.querySelectorAll('.product-details_product-details-list__2HUy4 li')
            );
            ...
            if (jsonDesc && detailInfo) {
                mainInfo = jsonDesc + '\n\n' + detailInfo;
            } else {
                mainInfo = jsonDesc || detailInfo;
            }
            ...
            const found = mainInfo.split('\n').find(text =>
                /^made\s*in/i.test(text.trim())
            );
        ...
        if (!productDetails || productDetails.soldOut) {
            console.warn('구찌 상품 품절 - 다음 productUrl로 이동');
            continue;
        }
        ...
        if (!productDetails || !productDetails.mainInfo || !productDetails.styleId || !productDetails.title || !productDetails.price || !productDetails.imageUrls.length || !productDetails.size || !productDetails.brandstyleId) {
            console.warn('구찌 데이터 누락 - 다음 productUrl로 이동');
...
const apiKey = account.apiKey;
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
        ...
        try {
            browser = await plainPuppeteer.launch({
                headless: true,
  ...
  `${customId}:${accountPlatform}:${goodsNo}`,
  {
            ...
            console.warn(`🚨 gucci 업데이트 실패: ${err.message}`);
        } finally {
            try {
                if (page && !page.isClosed()) {
